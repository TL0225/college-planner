...
function PlannerStatusPill({ state }: { state: PlannerStatusState }) {
  ...
  return (
    <span
      className="inline-flex shrink-0 items-center rounded-full px-[7px] py-[3px] text-[10px] font-semibold"
  ...
  return (
    <div className="min-h-0 flex-1 overflow-y-auto px-3 pb-3 pt-1">
      <div className="mx-auto w-full max-w-[720px] space-y-2">
        <div className="px-1 pb-0.5 pt-1 text-[11px] font-bold uppercase tracking-[0.06em] text-[var(--color-text-light)]">
          {headerLabel}
        </div>
        ...
        <button
          ...
          className="flex w-full items-center justify-center gap-2 rounded-xl border border-dashed px-3 py-3 text-[13px] font-medium text-[var(--color-text-light)] transition-colors hover:text-[var(--color-text-main)]"
          style={{
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 px-2.5 py-2">
        ...
        >
          <span className="truncate text-[13px] font-medium text-[var(--color-text-main)]">
            {displayTitle}
          </span>
          {semester.isCurrent ? (
            <span className="rounded-full bg-[color-mix(in_srgb,var(--color-primary)_14%,transparent)] px-1.5 py-0.5 text-[10px] font-semibold text-[var(--color-primary)]">
              Now
            </span>
          ) : null}
          <ChevronRight
        ...
        <span className="shrink-0 text-[12px] tabular-nums text-[var(--color-text-light)]">
        ...
        {hovered ? (
          <span className="flex shrink-0 items-center gap-0.5">
            <button
              ...
              onClick={(e) => {
                e.stopPropagation();
                showToast("Semester editing opens from Add semester for now", "info");
              }}
            >
      ...
      {expanded ? (
        <div className="border-t border-[var(--color-chrome-stroke)]">
          {courses.length === 0 ? (
            <div className="flex items-center gap-2 px-3 py-2.5 text-[12px] text-[var(--color-text-light)]">
...
function SemesterCourseLineRow({
  ...
  return (
    ...
    >
      ...
      <span className="min-w-0 flex-1 truncate text-[13px] text-[var(--color-text-main)]">
        {course.title}
      </span>
      {hovered ? (
        <button
