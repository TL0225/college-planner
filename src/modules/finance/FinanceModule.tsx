...
function accountTransactions(txs: Tx[], accountId: string, limit = CHART_TX_LIMIT) {
  return txs
...
function balanceSparklinePoints(account: Account, txs: Tx[], limit = CHART_TX_LIMIT) {
  const recent = accountTransactions(txs, account.id, limit);
  ...
  return points.length > 1 ? points : [account.balance, account.balance];
}

function TxSparkline({ points }: { points: number[] }) {
  const w = 280;
  const h = 56;
  const pad = 4;
  ...
  const tint = last >= first ? "var(--color-success)" : "var(--color-error)";
...
function TxBarChart({ txs }: { txs: Tx[] }) {
  if (txs.length === 0) {
    return (
      <p className="text-[12px] text-[var(--color-text-light)]">No transactions to chart yet.</p>
    );
  }
  const w = 280;
  const h = 56;
      ...
      {rows.map((row) => (
        <div key={row.label}>
          <div className="mb-1 flex items-center justify-between gap-2 text-[11px]">
            <span className="truncate text-[var(--color-text-light)]">{row.label}</span>
            <span className="shrink-0 tabular-nums text-[var(--color-text-main)]">
              {row.value.toLocaleString(undefined, { maximumFractionDigits: 0 })}
            </span>
          </div>
          <div className="h-1.5 overflow-hidden rounded-full bg-[var(--color-chrome-stroke)]">
            <div
              className="h-full rounded-full"
              style={{ width: `${(row.value / max) * 100}%`, background: tint }}
            />
          </div>
        </div>
      ))}
...
function CategoryDonutChart({ rows }: { rows: Array<[string, number]> }) {
    ...
    <div className="flex flex-wrap items-center gap-4">
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} role="img" aria-label="Spend by category">
        ...
        <text
          ...
          y={size / 2 - 4}
          textAnchor="middle"
          className="fill-[var(--color-text-main)]"
...
export function FinanceModule({ page = "dashboard" }: { page?: string }) {
  ...
  const load = useCallback(async () => {
    const [s, a, t, b, g, inv, rec, hold, recurringRows, dueRows] = await Promise.all([
  ...
  const totalSpend = txs.reduce((sum, t) => (t.amount < 0 ? sum + Math.abs(t.amount) : sum), 0);
  const categorySpendRows = [...spendByCategory.entries()].sort((a, b) => b[1] - a[1]);
  ...
  const selectedAccountTxs = useMemo(
    () =>
      selectedAccount
        ? accountTransactions(txs, selectedAccount.id).reverse()
        : [],
    [selectedAccount, txs],
  );
  const selectedAccountChartTxs = useMemo(
    () => (selectedAccount ? accountTransactions(txs, selectedAccount.id) : []),
    [selectedAccount, txs],
  );
      ...
      {view === "dashboard" && (
        <>
          <div className="grid gap-2.5 px-3 pt-1 md:grid-cols-4">
            ...
            <MetricTile label="Transactions" value={summary?.transactionCount ?? "—"} />
            <MetricTile label="Budgets" value={summary?.budgetCount ?? "—"} />
          </div>
          <div className="min-h-0 flex-1 overflow-auto p-3 pt-3">
            <div className="grid gap-3 lg:grid-cols-2">
                    ...
                    {accounts.map((a) => (
                      <li key={a.id}>
                        <ListRow
                          ...
                          trailing={
                            <span
                              className="text-[13px] font-semibold tracking-[-0.02em] tabular-nums text-[var(--color-text-main)]"
                            >
              ...
              <AppCard title="Dues & IOUs">
                {dues.filter((d) => !d.isPaid).length === 0 ? (
                  <EmptyState title="Nothing due" body="Open balances you owe or are owed appear here." />
                ) : (
                  <ul className="divide-y divide-[var(--color-chrome-stroke)]">
                    {dues
                      .filter((d) => !d.isPaid)
                        ...
                        <li key={d.id} className="flex items-center gap-2 py-1">
                          <div className="min-w-0 flex-1">
                            <ListRow
                              ...
                              subtitle={`Due ${new Date(d.dueAt).toLocaleDateString()}${d.notes ? ` · ${d.notes}` : ""}`}
                              trailing={
                          ...
                          <Button
                            ...
                            onClick={async () => {
                              try {
                                await ipc.financeMarkDuePaid(d.id);
              ...
              <AppCard title="Recent activity">
                {txs.length === 0 ? (
                  <EmptyState title="No transactions" body="Ledger entries show up here." />
                ) : (
                  <ul className="divide-y divide-[var(--color-chrome-stroke)]">
                    {txs.slice(0, 8).map((t) => (
                      <li key={t.id}>
            ...
            {(balanceByType.length > 0 || categorySpendRows.length > 0) && (
              <div className="mt-3 grid gap-3 lg:grid-cols-2">
                {balanceByType.length > 0 && (
                  <AppCard title="Net worth mix">
                    <MiniBarChart
                      rows={balanceByType.slice(0, 5).map(([label, value]) => ({
                        label,
                ...
                {categorySpendRows.length > 0 && (
                  <AppCard title="Top spending">
                    <MiniBarChart
                      rows={categorySpendRows.slice(0, 5).map(([label, value]) => ({
                        label,
                        value,
                      }))}
              ...
              <div className="flex h-full flex-col">
                ...
                >
                  <div className="flex flex-wrap items-center gap-1.5">
                    <StatusChip title={selectedAccount.accountType} filled />
                  </div>
                  <h3
                    className="mt-2 text-[var(--color-text-main)]"
                    style={{
                  ...
                  <p className="mt-0.5 text-[12px] text-[var(--color-text-light)]">
                    {selectedAccount.institution || "No institution"}
                  </p>
                  <p
                    className={`mt-3 text-[28px] font-semibold tracking-[-0.03em] tabular-nums ${
                      selectedAccount.balance < 0
                        ? "text-[var(--color-error)]"
                        : "text-[var(--color-text-main)]"
                    }`}
                  >
                    {money(selectedAccount.balance, selectedAccount.currency || "USD")}
                  </p>
                </div>
                <div className="min-h-0 flex-1 space-y-4 overflow-auto p-4">
                  <div>
                    ...
                    <TxSparkline
                      points={balanceSparklinePoints(selectedAccount, txs)}
                    />
                  </div>
                  <div>
                    <p className="mb-2 text-[11px] font-semibold uppercase tracking-[0.05em] text-[var(--color-text-light)]">
                    ...
                    <TxBarChart txs={selectedAccountChartTxs} />
                  </div>
                  <div>
                    <p className="mb-2 text-[11px] font-semibold uppercase tracking-[0.05em] text-[var(--color-text-light)]">
                      Recent transactions
                    ...
                    {selectedAccountTxs.length === 0 ? (
                      ...
                      <ul className="divide-y divide-[var(--color-chrome-stroke)] rounded-[10px] border border-[var(--color-chrome-stroke)]">
                        {selectedAccountTxs.slice(0, 8).map((t) => (
                          <li key={t.id}>
                            <ListRow
                              title={t.payee || "Untitled"}
                ...
                <div className="flex flex-wrap gap-2 border-t border-[var(--color-chrome-stroke)] p-3">
                  <Button size="sm" onClick={() => openAccountEditor(selectedAccount)}>
                    Edit
                  </Button>
                  <Button
                    size="sm"
                  ...
                  <Button size="sm" variant="ghost" onClick={closeAccountInspector}>
                    Close
                  </Button>
                </div>
              </div>
      ...
      {view === "reports" && (
        <>
          <div className="grid gap-2.5 px-3 pt-1 md:grid-cols-3">
            ...
            <MetricTile
              label="Total spend"
              value={txs.length ? money(totalSpend) : "—"}
            ...
            <div className="grid gap-3 lg:grid-cols-2">
              <AppCard title="Spend by category">
                {categorySpendRows.length === 0 ? (
                  <EmptyState
                    ...
                    body="Expense transactions appear here once you add ledger entries."
                  />
                ) : (
                  <div className="space-y-3">
                    <CategoryDonutChart rows={categorySpendRows} />
                    <ul className="divide-y divide-[var(--color-chrome-stroke)]">
                      {categorySpendRows.map(([category, spent]) => (
                        <li key={category}>
                          <ListRow
                            title={category}
                            leading={<StatusChip title={category} filled />}
                            trailing={
              ...
              <AppCard title="Budget progress">
                {budgets.length === 0 ? (
                  <EmptyState
                    title="No budgets"
                    body="Create budgets to compare targets against actual spend."
                  />
                ) : (
                  <ul className="divide-y divide-[var(--color-chrome-stroke)]">
                    {budgets.map((b) => {
