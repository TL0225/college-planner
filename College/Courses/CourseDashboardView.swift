...
struct CourseDashboardView: View {
    ...
    @FetchRequest(fetchRequest: CourseDashboardView.profileFetchRequest) private var profiles: FetchedResults<ProfileEntity>
    ...
    @FetchRequest private var courses: FetchedResults<CourseEntity>
    ...
    init(
        ...
        courseObjectID: NSManagedObjectID? = nil,
        onClose: @escaping () -> Void
    ) {
        self._activePage = activePage
        let normalized = Self.normalizeCourseCode(courseCode)
        self.courseCode = normalized
        ...
        courseRequest.sortDescriptors = []
        if let oid = courseObjectID {
            courseRequest.predicate = NSPredicate(format: "SELF == %@", oid)
        } else {
            courseRequest.predicate = NSPredicate(format: "code ==[c] %@", normalized)
        }
        _courses = FetchRequest(fetchRequest: courseRequest)
        ...
        if let oid = courseObjectID {
            let courseRef = context.object(with: oid)
            taskRequest.predicate = NSPredicate(format: "course == %@", courseRef)
        } else {
            taskRequest.predicate = NSPredicate(format: "course.code ==[c] %@", normalized)
        }
        ...
        eventRequest.fetchLimit = 40
        eventRequest.sortDescriptors = [NSSortDescriptor(key: "startDate", ascending: true)]
        if let oid = courseObjectID {
            let courseRef = context.object(with: oid)
            eventRequest.predicate = NSPredicate(format: "course == %@", courseRef)
        } else {
            eventRequest.predicate = NSPredicate(format: "course.code ==[c] %@", normalized)
        }
        _events = FetchRequest(fetchRequest: eventRequest)
    }

    private var course: CourseEntity? { courses.first }
    ...
    private var tagPills: [String] {
        ...
        if (course?.countsTowardGenEd ?? false) { pills.append("GenEd") }
    ...
    private var immediateActionTask: TaskEntity? {
        let now = Date()
    ...
    private var professorEmailText: String {
        (course?.professorEmail ?? "").trimmingCharacters(in: .whitespacesAndNewlines)
    }

    private var externalURLText: String {
        (coreDataManager.getCourseOverride(courseCode: courseCode)?.externalURL ?? "").trimmingCharacters(in: .whitespacesAndNewlines)
    }

    private var catalogDescriptionText: String {
        let raw = (course?.catalogCourse?.descriptionText ?? "").trimmingCharacters(in: .whitespacesAndNewlines)
        if CoreDataManager.isUnusableCatalogDescription(raw) { return "" }
        return raw
    }

    private var catalogPrerequisitesText: String {
        (course?.catalogCourse?.prerequisiteCodes ?? "").trimmingCharacters(in: .whitespacesAndNewlines)
    }

    private var catalogCorequisitesText: String {
        (course?.catalogCourse?.corequisiteCodes ?? "").trimmingCharacters(in: .whitespacesAndNewlines)
    }

    private var catalogTypicallyOfferedText: String {
        (course?.catalogCourse?.typicallyOffered ?? "").trimmingCharacters(in: .whitespacesAndNewlines)
    }
    ...
    private var nextUpcomingEvent: CalendarEventEntity? {
        let now = Date()
        ...
        let upcoming = events
            .filter { e in
                ...
                if end == nil, let start, start < now { return false }
                return true
            }
            .sorted { a, b in
    ...
    private var logisticsAnchorEvent: CalendarEventEntity? {
    ...
    private var logisticsIsHappeningNow: Bool {
        guard let e = logisticsAnchorEvent,
              let start = e.startDate,
              let end = e.endDate else {
            return false
        }
        let now = Date()
    ...
    private var logisticsMeetingDaysText: String? {
        guard let anchor = logisticsAnchorEvent, let anchorStart = anchor.startDate, let anchorEnd = anchor.endDate else {
            return nil
        }
        ...
        func isSimilarMeeting(_ e: CalendarEventEntity) -> Bool {
        ...
        for e in candidates {
            if let s = e.startDate {
                weekdaySet.insert(calendar.component(.weekday, from: s))
            }
        }
    ...
    private var logisticsMeetingTimeText: String? {
        guard let anchor = logisticsAnchorEvent, let start = anchor.startDate, let end = anchor.endDate else { return nil }
    ...
    private var logisticsUniversityHintName: String {
        let active = (coreDataManager.getActiveUniversity()?.name ?? "").trimmingCharacters(in: .whitespacesAndNewlines)
    ...
    private func presentAddTask() {
        modalCoordinator.courseDashboardTaskOverlay = .add(
            ...
            prefillCourseObjectID: course?.objectID
        )
    }

    private func presentEditTask(objectID: NSManagedObjectID) {
        modalCoordinator.courseDashboardTaskOverlay = .edit(objectID: objectID)
    }
        ...
        VStack(alignment: .leading, spacing: 16) {
                ...
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    if !relatedDocuments.isEmpty {
                        VStack(alignment: .leading, spacing: 8) {
                            ...
                            ForEach(relatedDocuments.prefix(6), id: \.objectID) { document in
                                HStack(spacing: 8) {
            ...
            Card {
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    let grading = course.map { coreDataManager.gradingCategories(for: $0) } ?? []
                    ...
                    if grading.isEmpty {
                        ...
                        VStack(spacing: 8) {
                            ForEach(grading.prefix(8), id: \.objectID) { item in
                                HStack {
                                    ...
                                    let w = (item.value(forKey: "weightPercent") as? Double) ?? (item.value(forKey: "weightPercent") as? NSNumber)?.doubleValue
                                    if let w, w > 0 {
    ...
    private var immediateAction: some View {
        HStack(alignment: .center, spacing: 18) {
            ...
            HStack(spacing: 12) {
                Button(action: {
                    if let t = immediateActionTask, let id = t.objectID as NSManagedObjectID? {
                        presentEditTask(objectID: id)
                    }
                }) {
                    Text("View Rubric")
                        ...
                        if upcomingItems.isEmpty {
                            ...
                            ForEach(showAllTasks ? Array(upcomingItems) : Array(upcomingItems.prefix(12)), id: \.id) { item in
                                DashboardTaskRow(
                                    ...
                                    onEdit: {
                                        switch item.kind {
                                        case .task(let objectID):
                                            presentEditTask(objectID: objectID)
                                        case .event(let objectID):
    ...
    private var syllabusSubtitle: String {
        let overrideEntity = coreDataManager.getCourseOverride(courseCode: courseCode)
    ...
    private var relatedDocuments: [VaultDocumentEntity] {
        coreDataManager.vaultDocuments.filter { document in
            ...
            return Self.normalizeCourseCode(linked) == courseCode
        }
    }
    ...
    private func computePrereqCheck() async {
        guard let catalogCourse = course?.catalogCourse else { prereqMet = nil; return }
        let context = coreDataManager.viewContext
        let request = NSFetchRequest<CourseEntity>(entityName: "CourseEntity")
        request.predicate = NSPredicate(format: "isCompleted == YES")
        let completedCourses = (try? context.fetch(request)) ?? []
        let validator = PrerequisiteValidator(coreDataManager: coreDataManager)
        let result = validator.validatePrerequisites(for: catalogCourse, completedCourses: completedCourses)
    ...
    fileprivate enum DashboardItemKind {
        case task(objectID: NSManagedObjectID)
        case event(objectID: NSManagedObjectID)
    }
    ...
    private var upcomingItems: [DashboardItem] {
        ...
        for t in tasks {
            let title = (t.title ?? "Task").trimmingCharacters(in: .whitespacesAndNewlines)
            let due = t.dueDate
            ...
            items.append(
                DashboardItem(
                    id: "task:\(t.objectID.uriRepresentation().absoluteString)",
                    title: title.isEmpty ? "Task" : title,
                    subtitle: subtitle,
                    dateText: dateText,
                    sortDate: due,
                    kind: .task(objectID: t.objectID),
                    badgeText: badgeText,
        ...
        for e in events {
            let title = (e.title ?? "Event").trimmingCharacters(in: .whitespacesAndNewlines)
            let start = e.startDate
            let end = e.endDate

            // Keep events that haven't ended yet.
            if let end, end < now { continue }
            if end == nil, let start, start < now { continue }

            let dateText = start.map { dateTimeFormatter.string(from: $0) } ?? "—"
            let startDay = start.map { calendar.startOfDay(for: $0) }
            let isPast = startDay.map { $0 < today } ?? false
            ...
            items.append(
                DashboardItem(
                    id: "event:\(e.objectID.uriRepresentation().absoluteString)",
                    title: title.isEmpty ? "Event" : title,
                    subtitle: subtitle,
                    dateText: dateText,
                    sortDate: start,
                    kind: .event(objectID: e.objectID),
                    badgeText: "Event",
    ...
    private func taskEffortText(for task: TaskEntity) -> String? {
        let rawMinutes = (task.value(forKey: "estimatedEffortMinutes") as? Int32).map(Int.init)
            ?? (task.value(forKey: "estimatedEffortMinutes") as? Int)
            ?? (task.value(forKey: "estimatedEffortMinutes") as? NSNumber)?.intValue
        guard let total = rawMinutes, total > 0 else { return nil }
        let hours = total / 60
    ...
    private func taskBadge(for task: TaskEntity) -> (text: String, color: Color) {
        let trimmed = (task.categoryName ?? "").trimmingCharacters(in: .whitespacesAndNewlines)
    ...
    private static func eventDateText(_ e: CalendarEventEntity) -> String {
        guard let start = e.startDate else { return "—" }
    ...
    private var scheduleSummaryText: String {
        guard let next = nextUpcomingEvent, let start = next.startDate else { return "" }
        ...
        if let end = next.endDate {
            let endText = Formatters.timeOnly.string(from: end)
...
private struct DashboardTaskRow: View {
    ...
    private var dueDateBlock: (date: String, weekday: String) {
        ...
        let referenceDate: Date? = {
            switch item.kind {
            case .task(let objectID):
                guard let task = (try? coreDataManager.viewContext.existingObject(with: objectID)) as? TaskEntity else { return item.sortDate }
    ...
    private var taskWeightText: String? {
        guard case .task(let objectID) = item.kind,
              let task = (try? coreDataManager.viewContext.existingObject(with: objectID)) as? TaskEntity else { return nil }
        let w = task.weightPercent
    ...
    private func toggleCompleted() {
        guard case .task(let objectID) = item.kind,
              let task = (try? coreDataManager.viewContext.existingObject(with: objectID)) as? TaskEntity else {
            return
        }
        coreDataManager.setTaskCompleted(task, completed: !task.isCompleted)
    }
