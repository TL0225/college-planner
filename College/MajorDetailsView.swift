...
struct MajorDetailsMainContent: View {
    ...
    private func genEdRows() -> [CourseRequirementRow] {
        ...
        if courses.isEmpty {
            return [
                .init(code: "", title: "No GenEd courses assigned.", credits: "-", grade: "-", semester: "-", status: .planned, isDimmed: false)
            ]
        }
    ...
    private func statusForCourse(code: String) -> RequirementStatus {
        ...
        // Default: course is not yet in the plan.
        return .notPlanned
    }
            ...
            if selectCount > 0 {
                ...
                if let selectDetailedJSON = req.selectFromDetailedJSON,
                   ...
                   !selectDetailed.isEmpty {
                    ...
                    let legacySelect = coreDataManager.decodeJSONCourseList(req.selectFromJSON)
    ...
    private func buildRows(from requirements: [DegreeRequirementEntity]) -> [CourseRequirementRow] {
        if isLoadingRequirements {
            return [
                .init(code: "", title: "Loading requirements…", credits: "-", grade: "-", semester: "-", status: .planned, isDimmed: false)
            ]
        }
        if let requirementsError {
            return [
                .init(code: "", title: requirementsError, credits: "-", grade: "-", semester: "-", status: .planned, isDimmed: false)
            ]
        }
        ...
        for req in requirements {
            ...
            if let detailedJSON = req.requiredCoursesDetailedJSON,
               ...
               !detailedCourses.isEmpty {
                for detail in detailedCourses {
                    let catalog = coreDataManager.getCatalogCourse(code: detail.code)
                    let planned = plannedCourse(for: detail.code)
                    let overrideEntity = courseOverride(for: detail.code)
                    let title = planned?.name ?? overrideEntity?.courseName ?? detail.title ?? catalog?.title ?? ""
                    let overrideCreditsText: String? = {
                        guard let overrideEntity else { return nil }
                        return overrideEntity.credits > 0 ? String(format: "%.1f", overrideEntity.credits) : nil
                    }()
                    let credits = planned.map { formatCredits($0.credits) }
                        ?? overrideCreditsText
                        ?? detail.credits
                    ...
                    rows.append(
                        .init(
                            ...
                            grade: grade.isEmpty ? "-" : grade,
                            semester: semester,
                            status: statusForCourse(code: detail.code),
                            isDimmed: false
                        )
                    )
                }
            ...
            if !requiredCodes.isEmpty {
                for code in requiredCodes {
                    ...
                    let title = planned?.name ?? overrideEntity?.courseName ?? catalog?.title ?? ""
                    let overrideCreditsText: String? = {
                        guard let overrideEntity else { return nil }
                        return overrideEntity.credits > 0 ? String(format: "%.1f", overrideEntity.credits) : nil
                    }()
            ...
            if let selectDetailedJSON = req.selectFromDetailedJSON,
               ...
               Int(req.selectCount) > 0 {
                let selectCount = Int(req.selectCount)
                let normalizedCodes = selectDetailed.map { $0.code }
                let isGroupSatisfied = statusForSelect(selectFrom: normalizedCodes, selectCount: selectCount) == .completed
                ...
                if selectCount == 1 {
                    for detail in selectDetailed {
                        ...
                        let grade = (planned?.grade ?? overrideEntity?.grade ?? "-")
                        let semester = planned.map(semesterText(for:)) ?? (overrideEntity?.semesterText ?? "-")
                        let status = statusForCourse(code: detail.code)
                        let dim = isGroupSatisfied && status != .completed
                    ...
                    let title = selectDetailed.map { detail in
                        if let courseTitle = detail.title {
                            return "\(detail.code): \(courseTitle)"
                        } else {
                            return detail.code
                        }
                    }.joined(separator: " / ")
                    ...
                    rows.append(
                        .init(
                            code: "SELECT \(selectCount)",
                            title: title,
                            credits: "-",
                            grade: "-",
                            semester: "-",
                            status: statusForSelect(selectFrom: selectDetailed.map { $0.code }, selectCount: selectCount),
                            isDimmed: false
                        )
                    )
                }
            ...
            if !selectFrom.isEmpty, selectCount > 0 {
                let isGroupSatisfied = statusForSelect(selectFrom: selectFrom, selectCount: selectCount) == .completed
                ...
                if selectCount == 1 {
                    for code in selectFrom {
                        ...
                        let grade = (planned?.grade ?? overrideEntity?.grade ?? "-")
                        let semester = planned.map(semesterText(for:)) ?? (overrideEntity?.semesterText ?? "-")
                        let status = statusForCourse(code: code)
                        let dim = isGroupSatisfied && status != .completed
                    ...
                    let title = selectFrom.joined(separator: " / ")
                    rows.append(
                        .init(
                            code: "SELECT \(selectCount)",
                            title: title,
                            ...
                            grade: "-",
                            semester: "-",
                            status: statusForSelect(selectFrom: selectFrom, selectCount: selectCount),
                            isDimmed: false
                        )
                    )
                }
        ...
        if rows.isEmpty {
            rows.append(.init(code: "", title: "No requirements found.", credits: "-", grade: "-", semester: "-", status: .planned, isDimmed: false))
        }
