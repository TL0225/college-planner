...
actor UniversalCatalogScraper {
    ...
    nonisolated private static func normalizeCourseCodeForLookup(_ raw: String) -> String? {
        let s = raw
          ...
          guard let re = try? NSRegularExpression(pattern: "\\b([A-Z]{2,6})\\s*(\\d{2,4})(?:[A-Z]{0,6})\\b") else { return nil }
    ...
    func scrapeAllPrograms(baseURL: URL, catalogID: Int) async throws -> [ScrapedProgram] {
        ...
        // 2. Scrape each container page
        for containerLink in containerLinks {
            logger.log("📄 Scraping container: \(containerLink.text)")
            let items = try await scrapeStream(url: containerLink.href, defaultContext: containerLink.text, catalogID: catalogID)
            for item in items {
        ...
        // Keep batches smaller to avoid overwhelming ModernCampus instances (UB has multiple catalogs).
        let batchSize = 4
        for batchStart in stride(from: 0, to: itemsArray.count, by: batchSize) {
            let batchEnd = min(batchStart + batchSize, itemsArray.count)
            let batch = Array(itemsArray[batchStart..<batchEnd])
                ...
                for item in batch {
                    group.addTask {
                        do {
                            ...
                            // Requirements are optional, but exports are requirement-driven.
                            // Log a small sample of failures to make debugging missing exports easier.
            ...
            let completed = min(batchEnd, itemsArray.count)
            let reqCount = programsWithRequirements.filter { $0.requirements != nil }.count
            logger.log("   ✓ \(completed)/\(itemsArray.count) programs processed (\(reqCount) with requirements)")
        }
    ...
    ///
    /// This is designed for on-demand usage when a user opens Major/Minor details.
    /// It parses Acalog's common requirement markup:
    /// - containers like `div.acalog-core`
    /// - headings (`h2/h3/h4`) used as requirement category labels
    /// - course list items (`li.acalog-course`)
    /// - adhoc list items (`li.acalog-adhoc`) often containing "OR" or notes
    func scrapeProgramRequirements(programURL: String) async throws -> [DegreeRequirement] {
        let result = try await scrapeProgramRequirementsWithDiagnostics(programURL: programURL)
        
        // Enhance course details with catalog cache (fillback for missing titles/credits)
        let enhancedRequirements = enhanceRequirementsWithCatalogData(result.requirements)
        
        return enhancedRequirements
    }

    func scrapeProgramRequirementsWithDiagnostics(programURL: String) async throws -> (requirements: [DegreeRequirement], diagnostics: ProgramRequirementsDiagnostics) {
        let logger = DebugLogger.shared
    ...
    nonisolated private static func parseProgramRequirementsWithDiagnostics(from doc: Document, logger: DebugLogger) throws -> (requirements: [DegreeRequirement], diagnostics: ProgramRequirementsDiagnostics) {
        ...
        func parsedCreditsRequired(from category: String) -> Int {
            ...
            guard let re = try? NSRegularExpression(pattern: "(?i)\\b(\\d+)\\s*credits?\\b") else { return 0 }
        ...
        func categoryLabel(for core: Element) -> String? {
            if let h = (try? core.select("h1,h2,h3,h4,h5,h6").first()), let t = try? h.text().normalizedCatalogText(), !t.isEmpty {
                return t
            }
            if let s = (try? core.select("strong,b").first()), let t = try? s.text().normalizedCatalogText(), !t.isEmpty, t.count <= 160 {
                return t
            }
        ...
        let signature: String = {
            let hasAcalogCore = ((try? doc.select("div.acalog-core").size()) ?? 0) > 0
            let hasAcalogCourse = ((try? doc.select("li.acalog-course").size()) ?? 0) > 0
            let hasPreviewCourseLinks = ((try? doc.select("a[href*=preview_course]").size()) ?? 0) > 0
            let hasPreviewCourseNoPopLinks = ((try? doc.select("a[href*=preview_course_nopop]").size()) ?? 0) > 0
            let hasShowCourseOnClick = ((try? doc.select("a[onClick*=showCourse],a[onclick*=showCourse]").size()) ?? 0) > 0
            ...
            for core in requirementsCoreBlocks {
                ...
                func applyRuleSignals(from text: String) {
                    ...
                    if t.range(of: "\\bselect\\b|\\bchoose\\b", options: [.regularExpression, .caseInsensitive]) != nil {
                        ...
                        if let re = try? NSRegularExpression(pattern: "(?i)\\b(?:select|choose)\\s+(\\d+)\\b") {
                ...
                if let items = try? core.select("li"), items.size() > 0 {
                    ...
                    for li in items {
                        // Avoid flattening nested lists (e.g., "Excluded courses") into one mega-line.
                        // Nested lists are typically explanatory content, not requirement rows.
                        ...
                        if li.hasClass("acalog-adhoc") {
                            applyRuleSignals(from: t)
                            continue
                        }
    ...
    nonisolated private static func extractCourseDetails(from text: String) -> [CourseDetail] {
        ...
        guard let codeRe = try? NSRegularExpression(pattern: "(?i)\\b([A-Z]{2,6})\\s*[-–]?\\s*([0-9]{2,4})([A-Z]{0,6})\\b") else {
            return []
        }
    ...
    nonisolated private static func parseTitleAndCredits(from text: String) -> (title: String?, credits: String?) {
        ...
        let creditPatterns = [
            "\\((\\d+(?:\\s*[-–]\\s*\\d+)?)(?:\\s*(?:credits?|crs?|hrs?))?\\)",  // (3), (3 credits), (1-6), (1-6 credits)
            "(?:credits?|crs?|hrs?)\\s*:?\\s*(\\d+(?:\\s*[-–]\\s*\\d+)?)",       // Credits: 3, Credits: 1-6
            "\\b(\\d+(?:\\s*[-–]\\s*\\d+)?)\\s*(?:credits?|crs?|hrs?)\\b",       // 3 credits, 1-6 credits, 4 cr
        ]
            ...
            if let re = try? NSRegularExpression(pattern: pattern, options: .caseInsensitive) {
                ...
                if let match = re.firstMatch(in: s, range: nsRange),
                   ...
                   let r = Range(match.range(at: 1), in: s) {
                    // Extract the credit string (e.g., "3" or "1-6")
                    let creditString = String(s[r]).replacingOccurrences(of: "\\s+", with: "", options: .regularExpression)
    ...
    private func buildProgramOwnershipOverridesFromDepartmentEntities(baseURL: URL, catalogID: Int) async throws {
        ...
        try await withThrowingTaskGroup(of: [(programURL: String, programName: String, dept: String, college: String)].self) { group in
    ...
    private func scrapeCoursesCatalog(baseURL: URL, catalogID: Int) async throws {
        ...
        // 3. Scrape course details in batches
        let batchSize = 10
        var scrapedCount = 0
        
        for batchStart in stride(from: 0, to: courseLinks.count, by: batchSize) {
            let batchEnd = min(batchStart + batchSize, courseLinks.count)
            let batch = Array(courseLinks.array()[batchStart..<batchEnd])
            
            try await withThrowingTaskGroup(of: CourseDetail?.self) { group in
                for link in batch {
                    group.addTask {
                        let courseDetailURL = Self.forceCatoid(try link.attr("abs:href"), catalogID: catalogID)
                        ...
                        do {
                            ...
                            // The parent <p> contains: <h1>...</h1> <hr>description<br><br>Credits: 3<br>...
                            guard let courseTitleH1 = try courseDoc.select("#course_preview_title").first() else {
                                return nil
                            }
                            
                            // Get the H1 text (code + title)
                            let h1Text = try courseTitleH1.text()
                                .replacingOccurrences(of: "\u{00A0}", with: " ")
                                .replacingOccurrences(of: "\\s+", with: " ", options: .regularExpression)
                                .trimmingCharacters(in: .whitespacesAndNewlines)
                            
                            // Parse course code from H1 text (e.g., "AAP 101SEM - Introduction to Arts Management")
                            // Pattern: [A-Z]{2,6} \d{3}[A-Z]* at the beginning
                            guard let codeRe = try? NSRegularExpression(pattern: "^([A-Z]{2,6})\\s+(\\d{3})([A-Z]*)"),
                            ...
                            // Extract title (text after "CODE - ")
                            // Format: "AAP 101SEM - Introduction to Arts Management"
                            var title: String?
                            ...
                            if let dashRange = afterCode.range(of: "-") {
                                title = String(afterCode[dashRange.upperBound...])
                                    .trimmingCharacters(in: .whitespacesAndNewlines)
                            ...
                            // Get parent <p> element to extract credits
                            guard let courseContainer = courseTitleH1.parent() else {
                                return nil
                            }
                            
                            let containerText = try courseContainer.text()
                                .replacingOccurrences(of: "\u{00A0}", with: " ")
                                .replacingOccurrences(of: "\\s+", with: " ", options: .regularExpression)
                            
                            // Extract credits from parent text
                            var credits: String?
                            if let creditsRe = try? NSRegularExpression(pattern: "Credits:\\s*(\\d+(?:\\s*-\\s*\\d+)?)", options: .caseInsensitive),
                            ...
                            // Only return course if we successfully extracted code and credits
                            ...
                            // Silently skip errors for individual courses
                            return nil
                        }
                    }
                }
            ...
            let completed = min(batchEnd, courseLinks.count)
            logger.log("   ✓ \(completed)/\(courseLinks.count) course pages processed (\(scrapedCount) cached)")
        }
    ...
    private func scrapeStream(url: String, defaultContext: String, catalogID: Int) async throws -> [HierarchyItem] {
        ...
        for el in stream {
            ...
            // If we still have no ownership at all, fall back to scraping the program page.
            if (department ?? "").isEmpty && (college ?? "").isEmpty {
                let extracted = try await extractDepartmentAndCollege(from: fixedHref)
                department = extracted.department?.normalizedCatalogText()
                college = extracted.college?.normalizedCatalogText()
            }
            ...
            if validURL.host?.contains("buffalo.edu") == true {
                let refined = refineUBOwnership(programName: programName, department: department, college: college)
                department = refined.department
                college = refined.college
            }

            let inferred = inferProgramTypeAndDegreeType(from: programName, contextHint: defaultContext)
            let item = HierarchyItem(
                programName: inferred.cleanedName,
                programURL: canonicalProgramURL,
                programType: inferred.programType,
