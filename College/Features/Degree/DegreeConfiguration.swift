// DegreeConfiguration.swift
// Feature: Degree
// Purpose: Degree module — DegreeLevel.
...
struct DegreeConfiguration {
    ...
    /// Built from [`DegreeTokenRegistry`] so picker UI and inference never diverge.
    static var degreeLevels: [DegreeLevel] {
        var levels = DegreeTokenRegistry.degreeLevelsFromRegistry()
        if let dentalIdx = levels.firstIndex(where: { $0.level == dentalSchool }) {
            var types = levels[dentalIdx].types
            if !types.contains("Advanced Dental Certificate") {
                types.append("Advanced Dental Certificate")
            }
            levels[dentalIdx] = DegreeLevel(level: dentalSchool, types: types)
        }
        if let medicalIdx = levels.firstIndex(where: { $0.level == medicalSchool }) {
            var types = levels[medicalIdx].types
            if !types.contains("Medical Certificate") {
                types.append("Medical Certificate")
            }
            levels[medicalIdx] = DegreeLevel(level: medicalSchool, types: types)
        }
        return levels
    }
    ...
    static func level(for degreeType: String) -> String? {
        ...
        if let fromRegistry = DegreeTokenRegistry.level(forFullLabelOrToken: normalizedType) {
            return fromRegistry
        }
        if let canonical = DegreeTypeNormalizer.normalize(normalizedType) {
            return canonical.degreeLevel
        }
    ...
    static func canonicalLevel(_ level: String) -> String {
        ...
        // Backward compatibility with legacy values previously stored in local store/UI.
