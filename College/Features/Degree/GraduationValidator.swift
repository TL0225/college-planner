...
// Feature: Degree
// Purpose: Degree module — GraduationValidationResult.
...
class GraduationValidator {
    ...
    func validateGraduationReadiness(
        for plan: PlannerPlan,
        university: University
    ) -> GraduationValidationResult {
        
        let major = (plan.major ?? "").trimmingCharacters(in: .whitespacesAndNewlines)
        let degreeType = plan.type.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !major.isEmpty, !degreeType.isEmpty else {
        ...
        if isUndergraduateDegreeType(degreeType) {
            ...
            categoryResults.append(
                CategoryResult(
                    category: "\(university.name) General Education",
                    creditsRequired: 0,
                    creditsCompleted: 0,
        ...
        // Check school policies — use a safe default since SchoolPolicies is not a local store
        // managed type and cannot be retrieved via legacy local store KVC.
    ...
    private func validateRequirementCategory(
        requirement: CatalogDegreeRequirement,
        ...
        let completedByCode: [String: PlannerCourse] = {
            var dict: [String: PlannerCourse] = [:]
        ...
        if selectCount > 0 {
            ...
            let completedMatches: [PlannerCourse] = selectCodes.compactMap { completedByCode[$0] }
            ...
            if !requiredCodes.isEmpty {
                ...
                for course in completedCourses {
                    let dept = requirement.requirementCategory
    ...
    private func validatePolicies(
        ...
        if let maxPerSemester = policies.maxCreditsPerSemester {
            ...
            for semester in semesters {
                let semesterCourses = semester.coursesArray
    ...
    private func getDegreeRequirements(
        ...
        guard let requirements = university.degreeRequirements else {
            return []
        }
    ...
    private func getAllCourses(from plan: PlannerPlan) -> [PlannerCourse] {
        plan.semestersArray.flatMap(\.coursesArray)
    }
    
    private func isMinorCourse(_ course: PlannerCourse, minor: String?) -> Bool {
        ...
        return course.code.uppercased().hasPrefix(minorPrefix)
    }
    
    // MARK: - Progress Tracking
    
    /// Get detailed progress breakdown by category
    func getProgressBreakdown(for plan: PlannerPlan, university: University) -> ProgressBreakdown {
