// DegreeView.swift
// Feature: Degree
// Purpose: Degree module — DegreeView.
...
struct DegreeView: View {
    ...
    @State private var selectedPlan: PlannerPlan?
    ...
    private var plans: [PlannerPlan] {
        ...
        return collegePersistence.plans
    }

    private var profile: Profile? {
        _ = plannerRefreshToken
        return collegePersistence.profile
    }
        ...
        VStack(spacing: 0) {
            ...
            HStack(spacing: 0) {
                ...
                if showMajorDetails {
                    ...
                    MajorMinorDetailsView(
                        majorDisplay: selectedMinorDisplay ?? (collegePersistence.resolvedMinorNames().first ?? ""),
                        onBack: {
                            hideAllDetails()
                        },
                        programKind: .minor
                    ...
                    DegreeMainContent(selectedPlan: $selectedPlan)
                }
            }
            ...
            .onReceive(NotificationCenter.default.publisher(for: .showMinorDetails)) { notification in
                if let minor = notification.object as? String {
                    ...
                    selectedMinorDisplay = collegePersistence.resolvedMinorNames().first
                }
                showDetails(.minor)
            }
            .onAppear {
            ...
            .onChange(of: selectedPlan) { _, newValue in
                collegePersistence.setActivePlan(newValue)
            }
    ...
    private func ensureDefaultPlanSelectedIfNeeded() {
        if let existing = plans.first {
            ...
            collegePersistence.setActivePlan(selectedPlan)
            return
        }

        // If the user has no plans, create a basic default plan.
        let plan = collegePersistence.addPlan(
            name: "My Plan",
            type: "Bachelors",
...
struct DegreeSidebar: View {
    ...
    private var collegeSubtitle: String {
        ...
        let dept = (collegePersistence.primaryDepartment() ?? "").trimmingCharacters(in: .whitespacesAndNewlines)

        if !university.isEmpty, !dept.isEmpty,
           let school = collegePersistence.schoolForDepartment(universityName: university, departmentName: dept) {
            return school
        }
    ...
    private var minorSubtitle: String {
        let minor = (collegePersistence.resolvedMinorNames().first ?? "").trimmingCharacters(in: .whitespacesAndNewlines)
        if minor.isEmpty || minor.lowercased() == "none" { return "None" }
        return minor
    }

    @State private var isRequirementsProgressLoading: Bool = false
    @State private var majorProgressCache: [String: CollegePersistence.CreditsProgressSummary] = [:]
    @State private var minorProgressCache: CollegePersistence.CreditsProgressSummary = CollegePersistence.CreditsProgressSummary(completed: 0, required: 0, fraction: 0)
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 24) {
                ...
                let gen = collegePersistence.genEdCreditsProgress(for: selectedPlan)

                let majors = selectedMajors.isEmpty ? ["Select Major"] : selectedMajors
                let majorSummaries = majors.map { majorProgressCache[$0] ?? CollegePersistence.CreditsProgressSummary(completed: 0, required: 0, fraction: 0) }
                ...
                let minorSummary = (minorSubtitle == "None")
                    ? CollegePersistence.CreditsProgressSummary(completed: 0, required: 0, fraction: 0)
                    : minorProgressCache
                ...
                ForEach(majors, id: \.self) { majorDisplay in
                    let p = majorProgressCache[majorDisplay] ?? CollegePersistence.CreditsProgressSummary(completed: 0, required: 0, fraction: 0)
        ...
        .task(id: progressComputationKey) {
            ...
            let degreeTypeRaw = (collegePersistence.primaryDegreeType() ?? "").trimmingCharacters(in: .whitespacesAndNewlines)
            let majors = selectedMajors.isEmpty ? ["Select Major"] : selectedMajors

            isRequirementsProgressLoading = true
            let start = Date()

            let majorMap = await collegePersistence.computeMajorRequirementsCreditsProgressSummariesAsync(
                majorDisplays: majors,
                degreeTypeRaw: degreeTypeRaw
            )

            let computedMinor: CollegePersistence.CreditsProgressSummary = {
                if minorSubtitle == "None" { return CollegePersistence.CreditsProgressSummary(completed: 0, required: 0, fraction: 0) }
                return collegePersistence.minorRequirementsCreditsProgress(forMinorDisplay: minorSubtitle)
            }()
    ...
    private var progressComputationKey: String {
        ...
        let degreeTypeKey = (collegePersistence.primaryDegreeType() ?? "").trimmingCharacters(in: .whitespacesAndNewlines)
                ...
                HStack {
                    ...
                    HStack(spacing: 16) {
                        ...
                        Button(action: {
                            ...
                            modalCoordinator.addSemesterPreferredPlanID = selectedPlan?.id
...
struct SemesterCard: View {
    ...
    @State private var inMemoryCourseOrder: [PlannerCourse] = []
    ...
    var body: some View {
        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            VStack(alignment: .leading, spacing: 16) {
                ...
                // Add Course Button
                Button(action: {
                    modalCoordinator.activeModal = .addCatalogCourse(semesterID: semester.id)
                }) {
        ...
        .onChange(of: draggingCourse?.id) { _, newValue in
            if newValue != nil {
    ...
    private func moveDroppedCourseIfPossible(payload: String) {
        guard let courseID = UUID(uuidString: payload.trimmingCharacters(in: .whitespacesAndNewlines)),
              let droppedCourse = try? collegePersistence.profileRepository.fetchCourse(id: courseID),
              let sourceSemester = droppedCourse.semester else { return }

        if sourceSemester.id == semester.id {
        ...
        let remaining = sourceSemester.coursesArray.filter { $0.id != droppedCourse.id }
...
struct CourseCard: View {
    ...
    @Binding var inMemoryCourseOrder: [PlannerCourse]
    ...
    var body: some View {
        HStack(alignment: .top) {
            VStack(alignment: .leading, spacing: 4) {
                ...
                Text("\(course.creditsInt) credits • \(course.status)")
        ...
        .onDrop(of: [UTType.text], delegate: CourseDropDelegate(destinationCourseID: course.id, semester: course.semester!, draggingCourse: $draggingCourse, collegePersistence: collegePersistence, isReorderingInMemory: $isReorderingInMemory, inMemoryCourseOrder: $inMemoryCourseOrder))
    }
}
...
struct CourseDropDelegate: DropDelegate {
    ...
    func dropEntered(info: DropInfo) {
          ...
          // Avoid mutating local store objects during drag-over; persist once on drop.
          guard isReorderingInMemory else { return }
        guard let draggingCourse,
              draggingCourse.semester == semester
        else { return }

          guard let fromIndex = inMemoryCourseOrder.firstIndex(where: { $0.id == draggingCourse.id }),
              let toIndex = inMemoryCourseOrder.firstIndex(where: { $0.id == destinationCourseID })
        else { return }
