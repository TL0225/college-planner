// LMSDownloadManager.swift
// Feature: LMS
// Purpose: LMS module — LMSDownloadManager.
// Data: CollegePersistence / repositories when applicable.

import Foundation
import WebKit

// MARK: - LMSDownloadManager
// WKDownloadDelegate that routes completed downloads to the Vault.

@MainActor
final class LMSDownloadManager: NSObject, WKDownloadDelegate {

    var onDownloadAdded: ((LMSDownload) -> Void)?
    ...
    func download(_ download: WKDownload,
        ...
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("LMSDownloads", isDirectory: true)
        ...
        do {
            ...
            await MainActor.run {
                _ = AppNotificationCenter.shared.post(
                    kind: .error,
                    title: String(localized: "lms.download.failed_title"),
                    message: String(format: String(localized: "lms.download.temp_dir_failed_format"), filename)
                )
            }
            return nil
        }
        ...
        let dl = LMSDownload(id: dlId, filename: filename, progress: 0, state: .downloading)
        onDownloadAdded?(dl)
        return dest
    }
    ...
    func downloadDidFinish(_ download: WKDownload) {
        guard let src = destinationURL else {
            Task { @MainActor in
                AppNotificationCenter.shared.post(
                    kind: .error,
                    title: String(localized: "lms.download.failed_title"),
                    message: String(format: String(localized: "lms.download.missing_file_format"), suggestedFilename)
                )
        ...
        Task {
            defer {
                Task { @MainActor in
                    self.onDownloadRowDismiss?(self.downloadId)
                }
            ...
            do {
                ...
                await MainActor.run {
                    _ = AppNotificationCenter.shared.post(
                        kind: .error,
                        title: String(localized: "lms.download.vault_failed_title"),
                        message: String(format: String(localized: "lms.download.vault_failed_format"),
                                        filename, detail)
                    )
                }
            }
    ...
    func download(_ download: WKDownload, didFailWithError error: Error, resumeData: Data?) {
        ...
        Task { @MainActor in
            ...
            AppNotificationCenter.shared.post(
                kind: .error,
                title: String(localized: "lms.download.failed_title"),
                message: String(format: String(localized: "lms.download.network_failed_format"),
                                filename, detail)
            )
        }
    }

    /// Prefer nested local store validation messages over the generic "Multiple validation errors occurred."
    ...
    private func importIntoVault(from url: URL) async throws {
        ...
        await MainActor.run {
            _ = AppNotificationCenter.shared.post(
                kind: .success,
                title: String(localized: "lms.download.saved_title"),
                message: String(format: String(localized: "lms.download.saved_message_format"), filename)
            )
        }
    }

    /// Best-effort copy for Finder visibility; vault remains the canonical encrypted store.
    nonisolated private static func copyToUserDownloadsFolder(from fileURL: URL, suggestedFilename: String) {
    ...
    static func inferCategory(filename: String) -> CollegePersistence.VaultDocumentCategory {
    ...
    /// Removes per-download UUID folders under `LMSDownloads` that were left behind (failed/cancelled)
    /// or are older than `maxAge` (default 24h). Safe to call from LMS tab `onAppear`.
    static func sweepStaleLMSDownloadFolders(maxAge: TimeInterval = 86_400) {
