// BrightspaceWebCoordinator.swift
// Feature: Brightspace
// Purpose: Brightspace module — BrightspaceImportItem.
...
struct BrightspaceImportItem: Identifiable {
    ...
    var pageType: BrightspacePageType
    var isSelected: Bool = true
}

struct BrightspaceDownload: Identifiable {
    let id: UUID
    var filename: String
    var progress: Double
...
struct BrightspaceCredentialOffer: Equatable {
    var username: String
    var host: String
}

// MARK: - BrightspaceWebCoordinator

@MainActor
final class BrightspaceWebCoordinator: NSObject, ObservableObject {
    ...
    @Published var pageTitle: String = "Brightspace"
    @Published var pageType: BrightspacePageType = .other
    @Published var pendingImportItems: [BrightspaceImportItem] = []
    @Published var activeDownloads: [BrightspaceDownload] = []
    @Published var offerSavePassword: BrightspaceCredentialOffer? = nil
    ...
    /// UserDefaults key for last successfully loaded Brightspace URL (Phase A session restore).
    static let lastVisitedURLStorageKey = "brightspace.lastVisitedURL"
    /// When true, next Brightspace tab appearance loads the portal and clears the resume bookmark (intents / menu).
    static let pendingLoadPortalKey = "brightspace.pendingLoadPortalOnNextAppear"
    private static let pageContextExtractionKey = "brightspace.allowPageContextExtraction"
    ...
    private var authPopupHost: BrightspaceAuthPopupHost?
    ...
    private func ensureWebViewCreated() {
        ...
        if let jsURL = Bundle.main.url(forResource: "BrightspaceJSBridge", withExtension: "js"),
    ...
    /// Loads the portal URL from Settings (or default); does not clear the last-visited bookmark.
    func loadPortalHome() {
        load(url: resolvePortalURL())
    }
    ...
    static func sanitizedDocumentTitle(_ raw: String?) -> String {
        let fallback = String(localized: "app.page.brightspace")
    ...
    /// Launch preload hook: proactively boots the web session so Brightspace tab entry is warm.
    func preloadPortalForLaunch(
        timeout: TimeInterval = 20,
        progress: ((Double) -> Void)? = nil,
        detail: ((String) -> Void)? = nil
    ) async throws {
        detail?("Initializing Brightspace web session")
        if webView.url == nil {
            detail?("Loading Brightspace portal")
            loadPortalHome()
            progress?(0.12)
        } else {
            detail?("Using existing Brightspace session")
            progress?(0.22)
        }
        ...
        while Date() < deadline {
            ...
            if hasURL && (hasMeaningfulProgress || !isLoading) {
                detail?("Brightspace preload complete")
                progress?(1)
                return
            }
        ...
        throw NSError(
            domain: "College.BrightspacePreload",
            code: 1,
            userInfo: [NSLocalizedDescriptionKey: "Timed out preloading Brightspace."]
        )
    }
    ...
    func resolvePortalURL() -> URL {
        let stored = UserDefaults.standard.string(forKey: "brightspace.portalURL") ?? ""
    ...
    #if DEBUG
    @MainActor
    func debugLogBrightspaceFocusSnapshot() {
        ...
        cookieStore.getAllCookies { cookies in
            let n = cookies.count
            Task { @MainActor in
                print("[Brightspace DEBUG] webView.url=\(wvURL) canGoBack=\(back) cookieCount=\(n)")
            }
        }
    }
    #endif
    ...
    private func injectSavedCredentialsForLoginPage(webView: WKWebView) async {
        ...
        guard let creds = BrightspaceKeychainService.shared.load(host: host) else { return }
...
extension BrightspaceWebCoordinator: WKNavigationDelegate {
    ...
    nonisolated func webView(_ webView: WKWebView, didStartProvisionalNavigation navigation: WKNavigation!) {
        Task { @MainActor [weak self] in
            ...
            if let path = webView.url?.path {
                let detectedType: BrightspacePageType
    ...
    private func assignDownloadDelegate(_ download: WKDownload) {
        ...
        let dlManager = BrightspaceDownloadManager()
        ...
        objc_setAssociatedObject(download, &BrightspaceWebCoordinator.dlManagerKey, dlManager, .OBJC_ASSOCIATION_RETAIN)
    }
...
extension BrightspaceWebCoordinator: WKUIDelegate {
    @MainActor
    func webView(_ webView: WKWebView,
        ...
        let host = BrightspaceAuthPopupHost(coordinator: self)
        authPopupHost = host
        return host.present(
    ...
    /// Shares cookies / process pool with the main Brightspace web view and wires the same JS bridge handlers.
    @MainActor
    ...
    fileprivate func clearAuthPopupHost(_ host: BrightspaceAuthPopupHost) {
...
extension BrightspaceWebCoordinator: WKScriptMessageHandler {
    ...
    private func handleScriptMessage(name: String, body: Any) {
        ...
        case "loginCredentials":
            ...
            let saveEnabled = UserDefaults.standard.object(forKey: "brightspace.savePassword") as? Bool ?? true
            guard saveEnabled else { return }
            offerSavePassword = BrightspaceCredentialOffer(username: username, host: host)
        default:
            break
        }
    }
    ...
    private func handlePageContext(_ dict: [String: Any]) {
        ...
        let newType = BrightspacePageType(rawValue: typeStr) ?? .other
        pageType = newType

        guard let rawItems = dict["items"] as? [[String: Any]], !rawItems.isEmpty else { return }

        var importItems: [BrightspaceImportItem] = []
        let df = ISO8601DateFormatter()

        for raw in rawItems {
            guard let title = raw["title"] as? String, !title.isEmpty else { continue }
            let id = raw["id"] as? String ?? UUID().uuidString
            let brightspaceItemId = raw["brightspaceItemId"] as? String ?? (currentURL?.absoluteString ?? "")
            var dueDate: Date?
            ...
            // so they survive the trip to BrightspaceImportSheet without adding new fields.
            ...
            let item = BrightspaceImportItem(
                id: id,
                title: title,
                dueDate: dueDate,
...
@MainActor
fileprivate final class BrightspaceAuthPopupHost: NSObject, NSWindowDelegate {
    private weak var coordinator: BrightspaceWebCoordinator?
    private var window: NSWindow?
    private(set) weak var popupWebView: WKWebView?

    init(coordinator: BrightspaceWebCoordinator) {
    ...
    func present(
        ...
        win.title = String(localized: "brightspace.auth_popup_title")
        win.center()
