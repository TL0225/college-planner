// GPACalculatorPopoverView.swift
// Feature: Courses
// Purpose: Courses module — GPACalculatorPopoverView.
...
struct GPACalculatorPopoverView: View {
    ...
    private func cumulativeGPASummary(mapping: [String: Double]) -> GPASemesterSummary? {
        ...
        for semester in semestersSorted {
            for course in semester.coursesArray {
                ...
                guard collegePersistence.isLetterGradedForGPA(course.gradingType) else { continue }
    ...
    private var semestersSorted: [SemesterEntity] {
        let semesters = collegePersistence.getActivePlan()?.semestersArray ?? collegePersistence.plans.flatMap { $0.semestersArray }
