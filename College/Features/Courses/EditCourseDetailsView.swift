// EditCourseDetailsView.swift
// Feature: Courses
// Purpose: Courses module — EditCourseDetailsView.
...
struct EditCourseDetailsView: View {
	...
	private var existingOverride: CourseOverrideEntity? {
		collegePersistence.getCourseOverride(courseCode: normalizedCourseCode)
	}

	private var plannedCourse: PlannerCourse? {
	...
	private var catalogCourse: CourseCatalog? {
		collegePersistence.getCatalogCourse(code: normalizedCourseCode)
	}
	...
	private var lastUpdatedDisplayText: String {
		...
		return formatDate(overrideEntity.lastUpdated)
	}
	...
	private func save() {
		...
		if !semesterYear.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
			collegePersistence.ensureCourseScheduledInPlanner(
				courseCode: normalizedCourseCode,
		...
		collegePersistence.upsertCourseInstructorContact(
			courseCode: normalizedCourseCode,
	...
	private func openVaultDocument(_ document: VaultDocumentEntity) {
		if let url = collegePersistence.urlForVaultDocument(document) {
		...
		case .success(let urls):
			...
			do {
				...
				let encryptedBookmark = securityManager.encryptBlobForStorage(bookmark)
					let didSave = collegePersistence.updateCourseOverrideSyllabus(
					courseCode: normalizedCourseCode,
					fileName: fileName,
