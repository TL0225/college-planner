...
struct CareerStatsView: View {
    ...
    @State private var careerSignal: CareerSignalSnapshot = CareerSignalSnapshot(
        scoredJobCount: 0,
        strongSkills: [],
        missingSkills: [],
        topCompanies: [],
    ...
    private func reloadStats() {
        ...
        careerSignal = CareerSignalAggregator.snapshot(collegePersistence: collegePersistence)
    }

    @ViewBuilder
    private var careerSignalCard: some View {
        statCard(title: "Career signal") {
            if careerSignal.scoredJobCount < CareerSignalAggregator.minimumScoredJobs {
                Text("Score \(CareerSignalAggregator.minimumScoredJobs)+ jobs to see patterns across your targets.")
                    .font(DesignSystem.Fonts.caption1())
                    .foregroundStyle(DesignSystem.Colors.textLight)
            } else {
                VStack(alignment: .leading, spacing: DesignSystem.Spacing.md) {
                    if !careerSignal.strongSkills.isEmpty {
                        Text("Skills you're strong on")
                            .font(DesignSystem.Fonts.caption1().weight(.semibold))
                        ForEach(careerSignal.strongSkills, id: \.skill) { row in
                            HStack {
                                Text(row.skill)
                                    .font(DesignSystem.Fonts.body())
                                Spacer()
                                Text("\(row.jobPercent)% of scored jobs")
                                    .font(DesignSystem.Fonts.caption1().monospacedDigit())
                                    .foregroundStyle(DesignSystem.Colors.textLight)
                            }
                        }
                    ...
                    if !careerSignal.missingSkills.isEmpty {
                        Text("Skills to develop")
                            .font(DesignSystem.Fonts.caption1().weight(.semibold))
                            .padding(.top, 4)
                        ForEach(careerSignal.missingSkills, id: \.skill) { row in
                            HStack {
                                Text(row.skill)
                                    .font(DesignSystem.Fonts.body())
                                Spacer()
                                Text("in \(row.jobCount) of \(row.totalJobs) jobs")
                                    .font(DesignSystem.Fonts.caption1().monospacedDigit())
                                    .foregroundStyle(DesignSystem.Colors.textLight)
                            }
                        }
                    ...
                    if !careerSignal.topCompanies.isEmpty {
                        Text("Best-matched companies")
                            .font(DesignSystem.Fonts.caption1().weight(.semibold))
                            .padding(.top, 4)
                        ForEach(careerSignal.topCompanies, id: \.company) { row in
                            HStack {
                                Text(row.company)
                                    .font(DesignSystem.Fonts.body())
                                Spacer()
                                Text("avg \(row.avgScore)% · \(row.jobCount) jobs")
                                    .font(DesignSystem.Fonts.caption1().monospacedDigit())
                                    .foregroundStyle(DesignSystem.Colors.textLight)
                            }
                        }
                    ...
                    if let bridge = careerSignal.topCourseBridge,
                       let code = bridge.courseCode {
                        Text("\(code) covers \(bridge.skill) — your most common gap.")
                            .font(DesignSystem.Fonts.caption1())
