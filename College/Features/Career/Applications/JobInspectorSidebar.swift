...
struct JobInspectorSidebar: View {
    ...
    @State private var matchRows: [CareerResumeMatchRow] = []
    @State private var isScoringMatch = false
    @State private var matchPanelState: BoardMatchPanelState = .idle
    ...
    var body: some View {
        ...
        .background(paneBackground)
        .onAppear { scheduleBoardMatchScoring() }
        .onChange(of: job.jobDescriptionText) { _, _ in scheduleBoardMatchScoring() }
    }

    private enum BoardMatchPanelState {
        case idle
        case loading
        case scored
    ...
    @ViewBuilder
    private var boardMatchPanel: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text("Resume match")
                .font(.caption.weight(.bold))
                .foregroundStyle(.secondary)

            switch matchPanelState {
            case .noJobDescription:
                Text("Job details not loaded — open from Openings to score this role.")
                    .font(.caption)
                    .foregroundStyle(.tertiary)
            case .loading:
                HStack(spacing: 8) {
                    ProgressView().controlSize(.small)
                    Text("Scoring resumes…")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
            case .scored:
                if let row = matchRows.first(where: \.isRecommended) ?? matchRows.first {
                    HStack {
                    ...
                    if let resume = job.submittedResume {
                        VStack(alignment: .leading, spacing: 2) {
                            Text("Submitted: \(resume.customDisplayName ?? resume.fileName)")
                                .font(.caption2)
                                .foregroundStyle(.secondary)
                            if let applied = job.dateApplied {
                                Text("Applied \(applied.formatted(date: .abbreviated, time: .omitted))")
                                    .font(.caption2)
                                    .foregroundStyle(.tertiary)
                            }
                        }
    ...
    private func scheduleBoardMatchScoring() {
        let jd = (job.jobDescriptionText ?? "").trimmingCharacters(in: .whitespacesAndNewlines)
        guard !jd.isEmpty else {
            matchPanelState = .noJobDescription
            return
        }

        matchPanelState = .loading
        Task {
            let postingSnapshot: CareerATSPostingSnapshot = await MainActor.run {
                if let posting = job.workdaySourcePosting {
                    return CareerATSPostingSnapshot(posting: posting)
                }
                return CareerATSPostingSnapshot(application: job)
            }
            let bundle = await CareerATSService.shared.scoreAllResumes(
                posting: postingSnapshot,
                platform: JobBoardPlatform(rawValue: job.source ?? "") ?? .workday,
                priority: .utility
            )
            await MainActor.run {
                matchRows = bundle.rows
                matchPanelState = bundle.rows.isEmpty ? .noJobDescription : .scored
            }
        }
    }
    ...
    @ViewBuilder
    private var applyInCollegeSection: some View {
        if let urlString = job.postingURLString,
           let url = URL(string: urlString),
           !urlString.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            Button("Apply in College") {
                let resumeID = job.submittedResume?.id ?? matchRows.first?.resumeDocumentID
                guard let resumeID else {
                    container.appNotifications.post(
                        kind: .warning,
                ...
                let platformRaw = job.source ?? JobBoardPlatform.workday.rawValue
                let platform = JobBoardPlatform(rawValue: platformRaw) ?? .workday
                CareerApplyLauncher.openApplyInCollege(
                    postingURL: url,
                    platform: platform,
                    resumeDocumentID: resumeID,
                    resumeFileName: job.resumeDisplayName ?? job.submittedResume?.fileName ?? "Resume",
                    companyName: job.company ?? "Company",
                    jobTitle: job.title ?? "Role",
                    jobApplicationID: job.id,
                    openWindow: openWindow,
                    collegePersistence: collegePersistence,
                    notifications: container.appNotifications
                )
            }
    ...
    private func openEditApplication() {
        container.careerNavigationRouter.editApplication(id: job.objectID)
    }
