...
struct CareerSharedResumeImportPayload: Codable, Sendable {
    let requestId: UUID
    let fileName: String
    let createdAt: Date
}
...
final class CareerIngestCoordinator {
    ...
    private let sharedResumeImportFileName = "career_shared_resume_import.json"
    private let sharedResumeFilePrefix = "career_shared_resume_"
    ...
    /// Share extension: stage a PDF in the app group and return the import request id.
    func writeSharedResumeImport(data: Data, fileName: String, requestId: UUID = UUID()) throws -> UUID {
        let stagedURL = sharedResumeFileURL(for: requestId)
        try data.write(to: stagedURL, options: [.atomic])
        let payload = CareerSharedResumeImportPayload(
            requestId: requestId,
            fileName: fileName,
            createdAt: Date()
        )
        try write(payload, to: sharedResumeImportFileName)
        postDarwin(name: "career.sharedResumeImportReady")
        return requestId
    }

    /// Main app: resolve a share-extension import to a staged PDF URL (consumes the payload).
    func consumeSharedResumeImport(requestId: UUID) -> URL? {
        guard let payload: CareerSharedResumeImportPayload = read(from: sharedResumeImportFileName),
              payload.requestId == requestId,
              Date().timeIntervalSince(payload.createdAt) <= ingestFreshnessInterval
        else {
            try? deleteSharedResumeImport(requestId: requestId)
            return nil
        }
        let stagedURL = sharedResumeFileURL(for: requestId)
        ...
        try? deleteFile(named: sharedResumeImportFileName)
        return stagedURL
    }

    func deleteSharedResumeImport(requestId: UUID) throws {
        try? deleteFile(named: sharedResumeImportFileName)
        let stagedURL = sharedResumeFileURL(for: requestId)
    ...
    private func sharedResumeFileURL(for requestId: UUID) -> URL {
        appGroupDirectory().appendingPathComponent("\(sharedResumeFilePrefix)\(requestId.uuidString).pdf")
    }
