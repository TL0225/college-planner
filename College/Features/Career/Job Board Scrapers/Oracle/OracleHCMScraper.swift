...
// Feature: Career / Job Board Scrapers / Oracle
// Purpose: Oracle HCM job board scraper and API models.
...
actor OracleHCMScraper: JobBoardScraper {
    ...
    func scrapeListings(
        ...
        while offset < maxAccumulatedJobs {
            ...
            var pageListings: [ScrapedJobListing] = []
            pageListings.reserveCapacity(items.count)
            for item in items {
                let id = item.id
                ...
                let listing = ScrapedJobListing(
                    ...
                    listingHash: JobListingHash.compute(title: item.title ?? "", locationsText: item.primaryLocation)
                )
                pageListings.append(listing)
                all.append(listing)
                if all.count >= maxAccumulatedJobs { break }
            }
    ...
    private func extractSiteNumber(from text: String) -> String? {
        for regex in SiteNumberPatterns.patterns {
