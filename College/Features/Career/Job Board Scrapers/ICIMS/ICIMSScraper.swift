// ICIMSScraper.swift
// Feature: Career / Job Board Scrapers / ICIMS
// Purpose: ICIMS / Jibe job board scraper and API models.
...
actor ICIMSScraper: JobBoardScraper {
        ...
        for jobURL in jobURLs.prefix(500) {
            if let id = jobId(from: jobURL) {
                listings.append(ScrapedJobListing(
                    ...
                    jobTypeText: nil,
                    timeType: nil,
                    listingHash: JobListingHash.compute(title: titleFromSlug(jobURL), locationsText: nil)
                ))
            }
        }
        if listings.isEmpty {
    ...
    private func scrapeSearchHTML(base: URL) async throws -> [ScrapedJobListing] {
        ...
        for match in regex.matches(in: html, range: range) {
            ...
            result.append(ScrapedJobListing(
                ...
                jobTypeText: nil,
                timeType: nil,
                listingHash: JobListingHash.compute(title: id, locationsText: nil)
            ))
        }
        return result
    }
    ...
    private func listing(from data: JibeJobData, base: URL) -> ScrapedJobListing {
        ...
        return ScrapedJobListing(
            ...
            timeType: nil,
            listingHash: JobListingHash.compute(title: data.title, locationsText: location)
        )
    }
    ...
    private func jibePostedFields(_ data: JibeJobData) -> (display: String?, date: Date?) {
        ...
        let parsed = JobBoardPostingParsing.parsePostedOn(raw)
            ?? JobBoardPostingParsing.parseISODate(raw)
        if let parsed {
