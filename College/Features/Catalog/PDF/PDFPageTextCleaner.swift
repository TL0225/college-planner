// PDFPageTextCleaner.swift
// Feature: Catalog
// Purpose: Catalog module — PDFPageTextCleaner.
// Data: CollegePersistence / repositories when applicable.

import Foundation

/// Strips repeated headers/footers from per-page line arrays (shared by syllabus and catalog PDF ingest).
enum PDFPageTextCleaner {
    ...
    static func stripRepeatedHeadersAndFooters(_ perPageLines: [[String]]) -> ([[String]], Int) {
        ...
        let repeatedHeaders = Set(headerCounts.filter { $0.value >= threshold }.map(\.key))
        let repeatedFooters = Set(footerCounts.filter { $0.value >= threshold }.map(\.key))
