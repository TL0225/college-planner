// ModernCampusCatalogLabels.swift
// Feature: Catalog
// Purpose: Catalog module — ModernCampusCatalogLabels.
// Data: CollegePersistence / repositories when applicable.

import Foundation

/// Shared normalization for Modern Campus / Acalog catalog titles so onboarding pickers and local store `degreeLevel` stay aligned.
enum ModernCampusCatalogLabels {
    /// Title as published on the school's catalog list (light cleanup only).
    static func postedDisplayTitle(from rawTitle: String) -> String {
        rawTitle
    ...
    /// Filters out navbar noise, archived editions, and other non-catalog rows from `catalog_list.php`.
    static func filterPostedCatalogs(from descriptors: [ModernCampusCatalogDescriptor]) -> [ModernCampusCatalogDescriptor] {
        descriptors.filter { !shouldExcludeCatalogListTitle($0.title) }
    }
    ...
    static func shouldExcludeCatalogListTitle(_ rawTitle: String) -> Bool {
        ...
        if lower.hasSuffix(" catalog home") { return true }
        if lower == "home" { return true }
        if lower.contains("handbook") && !lower.contains("catalog") { return true }
        if lower.contains("policies and procedures") { return true }
