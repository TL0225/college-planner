// AssistantWebFetchPolicy.swift
// Feature: Assistant
// Purpose: Assistant module — AssistantWebFetchPolicy.
...
enum AssistantWebFetchPolicy: Sendable {
    ...
    static func ensureCatalogRenderedFetchAllowed(for url: URL) throws {
        guard isURLAllowedForFetch(url) else {
            throw NSError(
                domain: "AssistantWebFetchPolicy",
                code: 1,
                userInfo: [NSLocalizedDescriptionKey: "Catalog host is not registered for rendered HTML fallback."]
            )
        }
    }
