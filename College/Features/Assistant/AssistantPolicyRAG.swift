// AssistantPolicyRAG.swift
// Feature: Assistant
// Purpose: Assistant module — AssistantPolicyContext.
...
actor AssistantPolicyRAGStore {
    ...
    /// Aligned with `AssistantWebMemoryStore` retention (on-device memory bounds).
    private static let pruneHighWaterMark = AssistantWebMemoryStore.retentionHighWaterMark
    private static let pruneTargetCount = AssistantWebMemoryStore.retentionTargetCount
    ...
    private func pruneIfNeeded() {
        guard chunks.count > Self.pruneHighWaterMark else { return }
        let drop = chunks.count - Self.pruneTargetCount
        guard drop > 0 else { return }
        chunks.sort { $0.retrievedAt < $1.retrievedAt }
