...
struct AcademicIdentityView: View {
    ...
    private func degreeLevelsForSchool(_ schoolName: String) -> [String] {
        ...
        guard coreDataManager.hasUniversityCatalog(name: trimmedSchool) else {
            return DegreeConfiguration.allLevels
        }

        let detected = DegreeConfiguration.allLevels.filter { level in
            !coreDataManager.fetchDepartments(for: trimmedSchool, degreeLevel: level).isEmpty
        }
        ...
        if let current = profile.degreeLevel?.trimmingCharacters(in: .whitespacesAndNewlines), !current.isEmpty {
            ...
            recessedCard {
                ...
                HStack(alignment: .top, spacing: 14) {
                    ...
                    VStack(alignment: .leading, spacing: 6) {
                        ...
                        Menu {
                            Button("Select standing…") { profile.classStanding = nil; coreDataManager.save() }
                            Divider()
                            ForEach(classStandings, id: \.self) { standing in
                                Button(standing) { profile.classStanding = standing; coreDataManager.save() }
                            }
                        } label: {
                            inputContainer { menuRow(value: profile.classStanding, placeholder: "Select standing…") }
                        }
                        .buttonStyle(.plain)
                    }
                }
            ...
            recessedCard {
                ...
                VStack(alignment: .leading, spacing: 10) {
                    ...
                    if hasSchool && !isEditingSchool {
                        let university = coreDataManager.getActiveUniversity()
                    ...
                    let syncDate = coreDataManager.getActiveUniversity()?.lastCatalogSync
                ...
                VStack(alignment: .leading, spacing: 16) {
                        ...
                        Menu {
                            ...
                            inputContainer {
                                if let dept = profile.department, !dept.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                                    ...
                                    menuRow(value: profile.major, placeholder: "Select department/program\u{2026}")
                                }
                            }
                        }
                        .buttonStyle(.plain)
                    ...
                    HStack(alignment: .top, spacing: 14) {
                        VStack(alignment: .leading, spacing: 6) {
                            ...
                            Menu {
                                ForEach(availableDegreeLevels, id: \.self) { level in
                                    Button(level) {
                                        profile.degreeLevel = DegreeConfiguration.canonicalLevel(level)
                                        let types = DegreeConfiguration.types(for: level)
                                        if !types.isEmpty { profile.degreeType = types[0] }
                                        let standings = DegreeConfiguration.classStandings(for: level)
                                        if let cur = profile.classStanding, !standings.contains(cur) {
                                ...
                                inputContainer {
                                    menuRow(value: profile.degreeLevel, placeholder: "Select degree level\u{2026}")
                                }
                            }
                            .buttonStyle(.plain)
                        }
                        ...
                        VStack(alignment: .leading, spacing: 6) {
                            ...
                            Menu {
                                ...
                                inputContainer {
                                    menuRow(value: profile.major, placeholder: "Select major\u{2026}")
                                }
                            }
                            .buttonStyle(.plain)
                        }
                    ...
                    HStack(alignment: .top, spacing: 14) {
                        VStack(alignment: .leading, spacing: 6) {
                            ...
                            Menu {
                                Button("None") { profile.secondaryMajor = nil; coreDataManager.save() }
                                if availableMajors.isEmpty {
                                    Button("No majors available") { }.disabled(true)
                                } else {
                                    ForEach(availableMajors, id: \.self) { major in
                                        Button(major) { profile.secondaryMajor = major; coreDataManager.save() }
                                    }
                                }
                            } label: {
                                inputContainer {
                                    menuRow(value: profile.secondaryMajor, placeholder: "Select major\u{2026}")
                                }
                            }
                            .buttonStyle(.plain)
                        }
                        ...
                        if DegreeConfiguration.isUndergraduate(profile.degreeLevel ?? "Undergraduate") {
                            VStack(alignment: .leading, spacing: 6) {
                                inputLabel("Minor")
                                Menu {
                                    Button("None") { profile.minor = nil; coreDataManager.save() }
                                    ForEach(availableMinors, id: \.self) { minor in
                                        Button(minor) { profile.minor = minor; coreDataManager.save() }
                                    }
                                } label: {
                                    inputContainer {
                                        menuRow(value: profile.minor, placeholder: "Select minor\u{2026}")
                                    }
                                }
                                .buttonStyle(.plain)
                            }
                            ...
                            VStack(alignment: .leading, spacing: 6) {
                                ...
                                Menu {
                                    ...
                                    inputContainer {
                                        menuRow(value: profile.minor, placeholder: "Select certificate\u{2026}")
                                    }
                                }
                                .buttonStyle(.plain)
                            }
                ...
                HStack(alignment: .top, spacing: 14) {
                    VStack(alignment: .leading, spacing: 6) {
                        ...
                        let concentrationBinding = Binding<String>(
                            get: { coreDataManager.plans.first?.concentration ?? "" },
                            set: { newVal in
                                if let plan = coreDataManager.plans.first {
                    ...
                    VStack(alignment: .leading, spacing: 6) {
                        ...
                        let transferBinding = Binding<String>(
                            get: { profile.transferGpa > 0 ? String(format: "%.2f", profile.transferGpa) : "" },
                            set: { newVal in
                                profile.transferGpa = Double(newVal) ?? 0.0
                                coreDataManager.save()
                            }
                        )
                        inputContainer {
    ...
    private func scrapeAndImportCatalog(school: SchoolManifest, forceRescrape: Bool = false) async {
        ...
        do {
            ...
            let presence = await coreDataManager.catalogPresence(universityName: school.name)
            ...
            if school.catalogFormat.lowercased() == "acalog", !catalogsToScrape.isEmpty {
                ...
                // Avoid capturing non-Sendable CoreDataManager inside TaskGroup tasks.
                // This value is invariant across catalogs for a given university.
                let lawBackfillNeeded = await MainActor.run {
                    coreDataManager.shouldForceCourseRescrapeForSubject(
            ...
            if programs.isEmpty, school.catalogFormat.lowercased() == "acalog", !programsWereSkippedByIncremental {
