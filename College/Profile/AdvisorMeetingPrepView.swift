...
struct AdvisorMeetingPrepView: View {
    ...
    private func generateHTML() async {
        ...
        let major = profile?.major ?? "Undeclared"
        let gpa = profile?.gpa ?? 0.0
        let creditsEarned = Int(profile?.creditsEarned ?? 0)
        let creditsRequired: Int = {
            guard let p = profile else { return 120 }
            return p.creditsRequired > 0 ? Int(p.creditsRequired) : 120
        }()
        let graduation = profile?.expectedGraduation ?? "—"
        let sap = coreDataManager.sapStats()

        // Build semester rows HTML
        var semesterHTML = ""
        let sortedSemesters = coreDataManager.semesters.sorted { a, b in
            if a.year != b.year { return a.year < b.year }
            return a.seasonOrder < b.seasonOrder
        }
        for sem in sortedSemesters {
            let semName = sem.name ?? "Semester"
