...
struct ProfileView: View {
    ...
    @State private var primaryMajorProgress = CoreDataManager.CreditsProgressSummary(completed: 0, required: 0, fraction: 0)

    @FetchRequest(fetchRequest: ProfileView.profileRequest)
    ...
    private var creditsEarnedDisplay: Int {
        ...
        return Int(profile?.creditsEarned ?? 0)
    }
    ...
    private var creditsRequiredDisplay: Int {
        ...
        return Int(profile?.creditsRequired ?? 0)
    }
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 24) {
                UnifiedActionHeader(
                    title: AppPage.profile.displayTitle,
                    subtitle: "Manage your identity, progress, and portfolio",
                    topPadding: 0,
                    horizontalPadding: 0,
                    bottomPadding: 8,
                ...
                HStack(alignment: .top, spacing: 24) {
                    ...
                    VStack(spacing: 24) {
                        ...
                        DegreeProgressCard(
                            ...
                            progressSubtitle: profile?.classStanding.nilIfEmpty
                        )
                    }
                    .frame(width: 320)
                }
        ...
        .task(id: "\(profile?.major ?? "")|\(profile?.objectID.uriRepresentation().absoluteString ?? "")") {
            loadPrimaryMajorProgress()
        }
        .onReceive(NotificationCenter.default.publisher(for: NSManagedObjectContext.didSaveObjectsNotification)) { notification in
            ...
            let relevant: Set<String> = [
                "ProfileEntity", "SemesterEntity", "CourseEntity",
                "DegreeRequirementEntity", "MajorEntity", "PlanEntity",
            ]
            let affected = Set(updatedTypes + insertedTypes + deletedTypes)
            guard !affected.isDisjoint(with: relevant) else { return }
            loadPrimaryMajorProgress()
        }
        ...
        .sheet(isPresented: $isAddingPortfolioProject) {
            if let profile {
                AddProjectSheet(profile: profile)
                    .environment(\.managedObjectContext, viewContext)
                    .dismissOnOutsideClickForSheet()
            }
        }
    ...
    private func loadPrimaryMajorProgress() {
        ...
        guard !major.isEmpty else {
            primaryMajorProgress = CoreDataManager.CreditsProgressSummary(completed: 0, required: 0, fraction: 0)
            return
        }
        primaryMajorProgress = coreDataManager.majorRequirementsCreditsProgress(forMajorDisplay: major)
    }
}
        ...
        ZStack(alignment: .topTrailing) {
            HStack(spacing: 20) {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    HStack(spacing: 16) {
                        metaPill(icon: "at", text: profile?.universityEmail.nilIfEmpty ?? String(localized: "profile.identity.add_email"), isPlaceholder: profile?.universityEmail.nilIfEmpty == nil)
                        metaPill(icon: "mappin.and.ellipse", text: profile?.permanentAddress.nilIfEmpty ?? String(localized: "profile.identity.add_location"), isPlaceholder: profile?.permanentAddress.nilIfEmpty == nil)
                    }
                    .padding(.top, 4)
                }
                Spacer()
