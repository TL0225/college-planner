...
// MARK: - Outlook import (file scope: `DateFormatter` + payload boxes for Core Data `perform` @Sendable closures)

private enum OutlookImportDateFormatters {
...
/// Bridges `[[String: Any]]` into `@Sendable` Core Data import work.
private final class OutlookGraphItemsBox: @unchecked Sendable {
    let items: [[String: Any]]
    init(_ items: [[String: Any]]) { self.items = items }
}

/// Collects remote→local ID map updates on a Core Data queue without mutating a captured `[String: String]`.
...
class CalendarIntegrationManager: ObservableObject {
    ...
    #if DEBUG
        nonisolated private static let perfLog = OSLog(
    ...
    private func setAppleSyncMap(_ map: [String: String]) {
        AppleCalendarIntegration.syncMap = map
        rebuildAppleReverseMap(from: map)
    }

    private func appleExternalID(forLocalID localID: String) -> String? {
    ...
    /// Removes one Google sub-calendar from the app: purges all its events from CoreData,
    ...
    func removeGoogleCalendar(_ calendar: ConnectedCalendar) {
        ...
        // Purge events from CoreData.
        if !localUUIDs.isEmpty {
            CoreDataManager.shared.bulkDeleteCalendarEvents(withUUIDs: localUUIDs)
        }
    ...
    /// Events remain in CoreData but are hidden (their toggleID is no longer enabled).
    ...
    func removeAcademicsCourse(_ calendar: ConnectedCalendar) {
        ...
        CoreDataManager.shared.removeAutoLinkedCourse(code: code)
        CoreDataManager.shared.notifyCalendarDidChange()
    }
    ...
    func sourceCalendarColor(for event: CalendarEventEntity) -> Color? {
        ...
        if Self.isCollegeAppManagedEvent(event) {
            let appleID: String = {
                if let code = event.course?.code?.trimmingCharacters(in: .whitespacesAndNewlines),
                    !code.isEmpty
                {
    ...
    private nonisolated static func isCollegeAppManagedEvent(_ event: CalendarEventEntity) -> Bool {
        guard event.entity.attributesByName["providerSource"] != nil else { return false }
        return (event.value(forKey: "providerSource") as? String) == "CollegeApp"
    }
    ...
    func disconnectAppleCalendar() {
        ...
        purgeAppleCalendarEventsFromCoreData()
    }
    ...
    @MainActor
    private func notifyCalendarDidChangeMainActor() {
        CoreDataManager.shared.notifyCalendarDidChange()
    }

    private func purgeAppleCalendarEventsFromCoreData() {
        let mapSnapshot = AppleCalendarIntegration.syncMap
        let localUUIDStrings = Array(mapSnapshot.values)
        let localUUIDs = localUUIDStrings.compactMap { UUID(uuidString: $0) }

        // Clear the Apple sync map.
        setAppleSyncMap([:])
        ...
        context.perform { [weak self] in
            ...
            fetch.predicate = NSPredicate(format: "id IN %@", localUUIDs)
            ...
            do {
                ...
                // Purge best-effort; ignore errors.
            }
        }
    }
    ...
    private func syncAppleCalendar(showNotifications: Bool) async {
        guard appleStatus == .connected else { return }
        ...
        await syncAppleEventsToCoreData(events, syncNotificationID: syncNotificationID)
    }

    private func syncAppleEventsToCoreData(_ events: [EKEvent], syncNotificationID: UUID?) async {
        ...
        let snapshots: [AppleEventSnapshot] = events.compactMap { ek in
        ...
        let mapUpdates: [String: String] = await context.perform {
            var updates: [String: String] = [:]

            // Batch pre-fetch all referenced local events in one SQL query.
            let applePreFetchUUIDs: [UUID] = snapshots.compactMap { snap in
                if let extracted = snap.localUUIDFromURL { return extracted }
                return (currentMap[snap.externalID]).flatMap { UUID(uuidString: $0) }
            }
            var applePreFetched: [UUID: CalendarEventEntity] = [:]
            if !applePreFetchUUIDs.isEmpty {
                let pfReq = NSFetchRequest<CalendarEventEntity>(entityName: "CalendarEventEntity")
                pfReq.predicate = NSPredicate(format: "id IN %@", applePreFetchUUIDs as [NSUUID])
                pfReq.returnsObjectsAsFaults = false
                if let fetched = try? context.fetch(pfReq) {
                    for ev in fetched { if let id = ev.id { applePreFetched[id] = ev } }
                }
            }

            func fetchLocalEvent(uuid: UUID) -> CalendarEventEntity? {
                if let cached = applePreFetched[uuid] { return cached }
            ...
            func findReusableUnmappedLocalEvent(
                title: String, isAllDay: Bool, start: Date, end: Date
            ) -> CalendarEventEntity? {
                ...
                let startPredicate = NSPredicate(
                    format: "startDate >= %@ AND startDate <= %@", startLower as NSDate,
                    startUpper as NSDate)
                let endPredicate = NSPredicate(
                    format: "endDate >= %@ AND endDate <= %@", endLower as NSDate,
                    endUpper as NSDate)

                request.predicate = NSCompoundPredicate(andPredicateWithSubpredicates: [
                    titlePredicate, allDayPredicate, startPredicate, endPredicate,
                ])
                guard let matches = try? context.fetch(request), !matches.isEmpty else {
                    return nil
                }
                for candidate in matches {
                    guard candidate.course == nil, candidate.semester == nil else { continue }
                    guard let cid = candidate.id?.uuidString.lowercased(),
                        !mappedLocalIDsLower.contains(cid)
                    else { continue }
                    return candidate
                }
                return nil
            ...
            for snap in snapshots {
                ...
                let urlString = snap.urlString
                let calendarIdentifier = snap.calendarIdentifier

                func setOptionalField(_ event: CalendarEventEntity, key: String, value: Any?) {
                    guard event.entity.attributesByName[key] != nil else { return }
                    event.setValue(value, forKey: key)
                }

                func applyAppleProviderFields(to event: CalendarEventEntity) {
                    let keepsCollegeOwnership = Self.isCollegeAppManagedEvent(event)
                    if !keepsCollegeOwnership {
                        setOptionalField(event, key: "providerSource", value: "AppleCalendar")
                    }
                    setOptionalField(event, key: "providerEventId", value: externalID)
                    setOptionalField(event, key: "providerAccountId", value: calendarIdentifier)
                    setOptionalField(event, key: "lastSyncedAt", value: Date())
                    setOptionalField(event, key: "lastSyncSource", value: "AppleCalendar")
                    setOptionalField(event, key: "descriptionMarkdown", value: notes)
                    setOptionalField(event, key: "descriptionHTML", value: nil)
                    setOptionalField(event, key: "conferenceURL", value: urlString)
                }

                // 1) If EKEvent is tagged with our URL, prefer that mapping.
                var localUUID: UUID?
                if let extracted = snap.localUUIDFromURL {
                    localUUID = extracted
                } else if let mappedLocalIDString = currentMap[externalID],
                    let uuid = UUID(uuidString: mappedLocalIDString)
                {
                    localUUID = uuid
                }

                if let localUUID, let local = fetchLocalEvent(uuid: localUUID) {
                    // Update local event from Apple calendar
                    if local.title != title || local.startDate != start || local.endDate != end
                        || local.allDay != isAllDay || local.location != location
                        || local.notes != notes
                    {
                        local.title = title.isEmpty ? "Event" : title
                        local.startDate = start
                        local.endDate = end
                        local.allDay = isAllDay
                        local.location = location
                        local.notes = notes
                        local.lastUpdated = Date()
                    }
                    applyAppleProviderFields(to: local)
                    updates[externalID] = localUUID.uuidString
                    continue
                }

                // 2) Try reuse to prevent duplicates.
                if let reusable = findReusableUnmappedLocalEvent(
                    title: title, isAllDay: isAllDay, start: start, end: end),
                    let existingID = reusable.id?.uuidString
                {
                    reusable.location = location
                    reusable.notes = notes
                    reusable.lastUpdated = Date()
                    applyAppleProviderFields(to: reusable)
                    updates[externalID] = existingID
                    continue
                }

                // 3) Create new local event.
                let newEvent = CalendarEventEntity(context: context)
                newEvent.id = UUID()
                newEvent.title = title.isEmpty ? "Event" : title
                ...
                newEvent.course = nil
                applyAppleProviderFields(to: newEvent)
    ...
    func exportEventToAppleCalendar(_ event: CalendarEventEntity) {
        exportEventToAppleCalendar(event, calendarName: nil)
    }

    /// Exports `event` to Apple Calendar, placing it in a calendar named `calendarName`
    /// (created under "My Calendars" if it does not yet exist). When `calendarName` is
    /// nil the shared "College" calendar is used.
    func exportEventToAppleCalendar(_ event: CalendarEventEntity, calendarName: String?) {
        guard appleStatus == .connected else { return }

        let localIDString = event.id?.uuidString ?? ""
        guard let localUUID = event.id else { return }
        ...
        Task(priority: .utility) { [weak self] in
            ...
            // Locate existing EKEvent by mapping, else create a new one.
    ...
    private static func appendCourseTagNotesIfNeeded(event: CalendarEventEntity) -> String? {
        ...
        // Preserve any user notes; add a stable tag block for future sync/matching.
    ...
    private func exportAllLocalEventsToGoogle(token: String) {
        ...
        context.perform { [weak self] in
            ...
            do {
                ...
                #if DEBUG
                    let message = "exportAllLocalEventsToGoogle(): fetch failed: \(error)"
                    Task { @MainActor [weak self] in
            ...
            Task { @MainActor [weak self] in
                ...
                #if DEBUG
                    self.debugLog(
                        "exportAllLocalEventsToGoogle(): exporting \(snapshots.count) events")
                #endif
    ...
    private func purgeGoogleCalendarEventsFromCoreDataAndClearState() {
        ...
        context.perform { [weak self] in
            ...
            if !localIDs.isEmpty {
                let mappedReq = NSFetchRequest<CalendarEventEntity>(
                    entityName: "CalendarEventEntity")
            ...
            if !googleFingerprints.isEmpty {
                let personalReq = NSFetchRequest<CalendarEventEntity>(
                    entityName: "CalendarEventEntity")
            ...
            do {
                ...
                #if DEBUG
                    self.debugLog(
                        "disconnectGoogle(): purge complete (deleted=\(deletedObjectIDs.count))")
                #endif
            } catch {
                #if DEBUG
    ...
    nonisolated private static func googleEventFingerprint(
        title: String?,
        start: Date?,
        end: Date?,
        allDay: Bool,
    ...
    private func syncGoogleEventsToCoreData(
        ...
        let makeRemoteKey: @Sendable (String, String) -> String = { calendarID, eventID in
            "\(calendarID)\(Self.googleRemoteKeySeparator)\(eventID)"
        }
        ...
        context.perform {
            ...
            // MARK: Enrichment helper — applies Google-specific metadata to a local event.
            func applyEnrichmentFields(_ event: CalendarEventEntity, from item: GoogleCalendarEventItem) {
                let keepsCollegeOwnership = Self.isCollegeAppManagedEvent(event)
                // Status (confirmed / tentative)
                event.setValue(item.status, forKey: "eventStatus")
                // Custom color
                event.setValue(item.colorId, forKey: "googleColorId")
                // Provider/source tracking
                if !keepsCollegeOwnership {
                    event.setValue("Google", forKey: "providerSource")
                }
                event.setValue(item.id, forKey: "providerEventId")
                event.setValue(calendarID, forKey: "providerAccountId")
                event.setValue(Date(), forKey: "lastSyncedAt")
                event.setValue("Google", forKey: "lastSyncSource")
                // Rich descriptions
                event.setValue(item.description, forKey: "descriptionMarkdown")
                event.setValue(item.description, forKey: "descriptionHTML")
                // Recurrence rule
                if let rrule = GoogleAttendeeHelper.rrule(from: item.recurrence) {
                    event.setValue(rrule, forKey: "recurrenceRule")
                }
                // Conference / video join link
                if let url = item.conferenceData?.entryPoints?.first(where: { $0.entryPointType == "video" })?.uri
                    ?? item.conferenceData?.entryPoints?.first?.uri {
                    event.setValue(url, forKey: "conferenceURL")
                }
                // Full conference payload JSON
                if let conferenceData = item.conferenceData,
                   let jsonData = try? JSONEncoder().encode(conferenceData),
                   let jsonStr = String(data: jsonData, encoding: .utf8) {
                    event.setValue(jsonStr, forKey: "conferenceDataJSON")
                }
                // Attendees JSON
                if let attendees = item.attendees, !attendees.isEmpty,
                   let jsonData = try? JSONEncoder().encode(attendees),
                   let jsonStr = String(data: jsonData, encoding: .utf8) {
                    event.setValue(jsonStr, forKey: "attendeesJSON")
                }
                // Reminders JSON
                if let reminders = item.reminders,
                   let jsonData = try? JSONEncoder().encode(reminders),
                   let jsonStr = String(data: jsonData, encoding: .utf8) {
                    event.setValue(jsonStr, forKey: "remindersJSON")
                }
            }

            // Batch pre-fetch all referenced local events in a single SQL query (vs. N individual fetches).
            let batchLookupUUIDs: [UUID] = items.compactMap { item in
                let rKey = makeRemoteKey(calendarID, item.id)
                let legacyKey = item.id
                let localStr =
                    currentMap[rKey] ?? (calendarID == "primary" ? currentMap[legacyKey] : nil)
                return localStr.flatMap { UUID(uuidString: $0) }
            }
            var preFetchedLocalEvents: [UUID: CalendarEventEntity] = [:]
            if !batchLookupUUIDs.isEmpty {
                let prefetchReq = NSFetchRequest<CalendarEventEntity>(
                    entityName: "CalendarEventEntity")
                prefetchReq.predicate = NSPredicate(
                    format: "id IN %@", batchLookupUUIDs as [NSUUID])
                prefetchReq.returnsObjectsAsFaults = false
                if let fetched = try? context.fetch(prefetchReq) {
                    for ev in fetched { if let id = ev.id { preFetchedLocalEvents[id] = ev } }
                }
            }

            // Pre-fetch all unmapped local events once — replaces O(N²) per-event CoreData queries
            // in findReusableUnmappedLocalEvent with a single fetch + in-memory lookup.
            var unmappedLocalByTitle: [String: [CalendarEventEntity]] = [:]
            let unmappedReq = NSFetchRequest<CalendarEventEntity>(entityName: "CalendarEventEntity")
            unmappedReq.predicate = NSPredicate(format: "course == nil AND semester == nil")
            unmappedReq.returnsObjectsAsFaults = false
            if let unmapped = try? context.fetch(unmappedReq) {
                for ev in unmapped {
            ...
            func findReusableUnmappedLocalEvent(
                ...
                // In-memory lookup — no additional CoreData query per call.
                let candidates = unmappedLocalByTitle[title.lowercased()] ?? []
                for candidate in candidates {
                    guard candidate.allDay == isAllDay else { continue }
                    guard let cs = candidate.startDate, abs(cs.timeIntervalSince(start)) <= 1 else { continue }
                    guard let ce = candidate.endDate,  abs(ce.timeIntervalSince(end))   <= 1 else { continue }
                    guard candidate.course == nil, candidate.semester == nil else { continue }
                    guard let cid = candidate.id?.uuidString.lowercased(),
                        !mappedLocalIDsLower.contains(cid)
                    else { continue }
            ...
            func fetchLocalEvent(uuid: UUID) -> CalendarEventEntity? {
                if let cached = preFetchedLocalEvents[uuid] { return cached }
                // Fallback: fetch individually if somehow not in the pre-fetch batch.
                let req = NSFetchRequest<CalendarEventEntity>(entityName: "CalendarEventEntity")
                req.predicate = NSPredicate(format: "id == %@", uuid as CVarArg)
                req.fetchLimit = 1
                return try? context.fetch(req).first
            ...
            for item in items {
                let remoteKey = makeRemoteKey(calendarID, item.id)
                let legacyKey = item.id

                // Handle Google-cancelled events (delta sync sends them with status="cancelled").
                if item.status == "cancelled" {
                    let mappedStr = currentMap[remoteKey] ?? (calendarID == "primary" ? currentMap[legacyKey] : nil)
                    ...
                    mapRemovals.append(remoteKey)
                    if calendarID == "primary" { mapRemovals.append(legacyKey) }
                    cancelledRemoteKeys.append(remoteKey)
                    continue
                }
                ...
                if deletedTombstones.contains(remoteKey)
                    || (calendarID == "primary" && deletedTombstones.contains(legacyKey))
                {
                    continue
                }
                ...
                if let dt = item.start.dateTime {
                    startDate = Self.parseISO8601(dt)
                } else if let d = item.start.date {
                    startDate = Self.parseYMD(d)
                    isAllDay = true
                }

                if let dt = item.end.dateTime {
                    endDate = Self.parseISO8601(dt)
                } else if let d = item.end.date {
                    endDate = Self.parseYMD(d)
                }
                ...
                let mappedLocalIDString =
                    currentMap[remoteKey] ?? (calendarID == "primary" ? currentMap[legacyKey] : nil)
                    ...
                    let localEvent = fetchLocalEvent(uuid: uuid)
                {
                    ...
                    if currentMap[remoteKey] == nil, calendarID == "primary",
                        currentMap[legacyKey] != nil
                    {
                    ...
                    if localEvent.title != title || localEvent.startDate != finalStart
                        || localEvent.endDate != finalEnd || localEvent.allDay != isAllDay
                        || localEvent.location != location || localEvent.notes != notes
                    {
                    ...
                    // Always re-apply enrichment fields (color, recurrence, conference, attendees may change).
                    applyEnrichmentFields(localEvent, from: item)
                }
                ...
                if !eventExists {
                        ...
                        let existingID = reusable.id?.uuidString
                    {
                        ...
                        newEvent.semester = nil
                        newEvent.course = nil
                        applyEnrichmentFields(newEvent, from: item)
            ...
            Task { @MainActor [weak self] in
                guard let self else {
                    #if DEBUG
                        os_signpost(.end, log: log, name: "GoogleCoreDataSync", signpostID: spid)
                    #endif
                    completion()
                    return
                }
                ...
                // Tombstone Google-cancelled events so a full re-sync won't resurrect them.
                if !cancelledRemoteKeys.isEmpty {
                    var existing = Set(self.deletedIDs)
                    for key in cancelledRemoteKeys { existing.insert(key) }
                    ...
                    if let saveError {
                        self.debugLog(
                            "syncGoogleEventsToCoreData: background save FAILED: \(saveError)")
                    } else {
                ...
                if let syncNotificationID = syncNotificationID {
                    if newCount > 0 {
                        AppNotificationCenter.shared.complete(
                            ...
                            message:
                                "\(newCount) new event\(newCount == 1 ? "" : "s") synced from Google Calendar",
                            autoDismissAfter: 4
                        )
                    } else {
                        AppNotificationCenter.shared.complete(
    ...
    /// Bulk version: single SQL query for a list of UUIDs → O(1) per-event lookup.
    private func fetchLocalEventsBatch(uuids: [UUID]) -> [UUID: CalendarEventEntity] {
        guard !uuids.isEmpty else { return [:] }
        let context = CoreDataManager.shared.viewContext
        let req = NSFetchRequest<CalendarEventEntity>(entityName: "CalendarEventEntity")
        req.predicate = NSPredicate(format: "id IN %@", uuids as [NSUUID])
        req.returnsObjectsAsFaults = false
        guard let events = try? context.fetch(req) else { return [:] }
        var result: [UUID: CalendarEventEntity] = [:]
        result.reserveCapacity(events.count)
        for ev in events { if let id = ev.id { result[id] = ev } }
        return result
    }

    func exportEventToGoogle(_ event: CalendarEventEntity, targetCalendarID: String? = nil) {
        // 1. Check Auth
        guard googleStatus == .connected else { return }
        ...
        Task { [weak self] in
            ...
            do {
                ...
                #if DEBUG
                    self.debugLog(
                        "Skipping Google Export (no token): \(error.localizedDescription)")
                #endif
            }
        }
    }
    ...
    func disconnectOutlook() {
        ...
        purgeOutlookEventsFromCoreData()
    ...
    private var outlookSyncMap: [String: String] {
        get { _outlookSyncMap }
        set {
    ...
    private func syncOutlook(showNotifications: Bool) async {
        guard outlookStatus == .connected else { return }
    ...
    private func fetchOutlookEvents(token: String, syncNotificationID: UUID?) async {
        ...
        for calID in calendarIDs {
            ...
            await syncOutlookEventsToCoreData(
    ...
    private func syncOutlookEventsToCoreData(
        _ items: [[String: Any]], calendarID: String, syncNotificationID: UUID?
    ) async {
        // Use a background context to avoid blocking the main thread during bulk Outlook import.
        let context = CoreDataManager.shared.container.newBackgroundContext()
        context.mergePolicy = NSMergePolicy.mergeByPropertyObjectTrump
        let currentMap = outlookSyncMap
        let itemsBox = OutlookGraphItemsBox(items)
        let mapBox = CalendarSyncMapMutationBox()

        await withCheckedContinuation { (continuation: CheckedContinuation<Void, Never>) in
            context.perform {
                let items = itemsBox.items
                ...
                // Batch pre-fetch all existing local events in one query.
                let preUUIDs: [UUID] = items.compactMap { item in
                    guard let rid = item["id"] as? String, let str = currentMap[rid] else {
                        return nil
                    }
                    return UUID(uuidString: str)
                }
                var preFetched: [UUID: CalendarEventEntity] = [:]
                if !preUUIDs.isEmpty {
                    let pfReq = NSFetchRequest<CalendarEventEntity>(
                        entityName: "CalendarEventEntity")
                    pfReq.predicate = NSPredicate(format: "id IN %@", preUUIDs as [NSUUID])
                    pfReq.returnsObjectsAsFaults = false
                    if let evs = try? context.fetch(pfReq) {
                        for ev in evs { if let id = ev.id { preFetched[id] = ev } }
                    }
                }

                for item in items {
                    guard let remoteID = item["id"] as? String else { continue }
                    ...
                    let bodyContentType =
                        ((item["body"] as? [String: Any])?["contentType"] as? String)?
                        .uppercased()
                    ...
                    func applyOutlookProviderFields(to event: CalendarEventEntity) {
                        let markdownDescription = bodyPreview?.isEmpty == false ? bodyPreview : body
                        let htmlDescription = bodyContentType == "HTML" ? body : nil
                        let keepsCollegeOwnership = Self.isCollegeAppManagedEvent(event)
                        if !keepsCollegeOwnership {
                            setOptionalField(event, key: "providerSource", value: "Outlook")
                        }
                        setOptionalField(event, key: "providerEventId", value: remoteID)
                        setOptionalField(event, key: "providerAccountId", value: calendarID)
                        setOptionalField(event, key: "lastSyncedAt", value: Date())
                        setOptionalField(event, key: "lastSyncSource", value: "Outlook")
                        setOptionalField(event, key: "descriptionMarkdown", value: markdownDescription)
                        setOptionalField(event, key: "descriptionHTML", value: htmlDescription)
                        setOptionalField(event, key: "conferenceURL", value: conferenceURL)
                    }

                    guard let start = parseDate(sDict, tz: tz), let end = parseDate(eDict, tz: tz)
                    else { continue }

                    if let idStr = currentMap[remoteID], let uuid = UUID(uuidString: idStr),
                        let ev = preFetched[uuid]
                    {
                        ev.title = title
                        ev.startDate = start
                        ev.endDate = end
                        ...
                        ev.notes = body
                        ev.lastUpdated = Date()
                        applyOutlookProviderFields(to: ev)
                    } else {
                        let ev = CalendarEventEntity(context: context)
                        ev.id = UUID()
                        ev.title = title
                        ev.startDate = start
                        ev.endDate = end
                        ...
                        ev.lastUpdated = Date()
                        applyOutlookProviderFields(to: ev)
                        if let newID = ev.id { mapBox.set(remoteID, newID.uuidString) }
                    }
                }
                try? context.save()
                let mapSnapshot = mapBox.snapshot
                Task { @MainActor [weak self] in
                    guard let self else { return }
                    var newMap = self.outlookSyncMap
                    for (k, v) in mapSnapshot { newMap[k] = v }
                    self.outlookSyncMap = newMap
                    self.notifyCalendarDidChangeMainActor()
                }
                continuation.resume()
            }
    ...
    private func purgeOutlookEventsFromCoreData() {
        let map = outlookSyncMap
        let ids = map.values.compactMap { UUID(uuidString: $0) }
        outlookSyncMap = [:]
        ...
        ctx.perform { [weak self] in
            ...
            if let res = try? ctx.execute(req) as? NSBatchDeleteResult,
                let objIDs = res.result as? [NSManagedObjectID], !objIDs.isEmpty
            {
                NSManagedObjectContext.mergeChanges(
                    fromRemoteContextSave: [NSDeletedObjectsKey: objIDs], into: [ctx])
            }
            Task { @MainActor [weak self] in
                self?.notifyCalendarDidChangeMainActor()
            }
    ...
    private func synciCloud(showNotifications: Bool) async {
        guard iCloudStatus == .connected else { return }
        ...
        do {
            ...
            for calURL in enabledURLs {
                ...
                await synciCloudEventsToCoreData(events, calendarURLString: calURL.absoluteString)
            }
            if let id = notifID {
                await MainActor.run {
                    AppNotificationCenter.shared.complete(
    ...
    private var iCloudSyncMap: [String: String] {
        get { _iCloudSyncMap }
        set {
    ...
    private func synciCloudEventsToCoreData(_ events: [iCloudEventData], calendarURLString: String)
        async
    {
        // Use a background context to avoid blocking the main thread during bulk iCloud import.
        let context = CoreDataManager.shared.container.newBackgroundContext()
        context.mergePolicy = NSMergePolicy.mergeByPropertyObjectTrump
        ...
        await withCheckedContinuation { (continuation: CheckedContinuation<Void, Never>) in
            context.perform {
                // Batch pre-fetch existing mapped events in one SQL query.
                let preUUIDs: [UUID] = events.compactMap { ev in
                    let mapKey = "\(calendarURLString)||\(ev.uid)"
                    return currentMap[mapKey].flatMap { UUID(uuidString: $0) }
                }
                var preFetched: [UUID: CalendarEventEntity] = [:]
                if !preUUIDs.isEmpty {
                    let pfReq = NSFetchRequest<CalendarEventEntity>(
                    ...
                    pfReq.returnsObjectsAsFaults = false
                    if let fetched = try? context.fetch(pfReq) {
                        for entity in fetched { if let id = entity.id { preFetched[id] = entity } }
                    }
                }
                ...
                for ev in events {
                    ...
                    func applyICloudProviderFields(to event: CalendarEventEntity) {
                        let keepsCollegeOwnership = Self.isCollegeAppManagedEvent(event)
                        if !keepsCollegeOwnership {
                            setOptionalField(event, key: "providerSource", value: "iCloudCalDAV")
                        }
                        setOptionalField(event, key: "providerEventId", value: ev.uid)
                        setOptionalField(event, key: "providerAccountId", value: calendarURLString)
                        setOptionalField(event, key: "lastSyncedAt", value: Date())
                        setOptionalField(event, key: "lastSyncSource", value: "iCloudCalDAV")
                        setOptionalField(event, key: "descriptionMarkdown", value: ev.notes)
                        setOptionalField(event, key: "descriptionHTML", value: nil)
                    }

                    if let idStr = currentMap[mapKey], let uuid = UUID(uuidString: idStr),
                        let entity = preFetched[uuid]
                    {
                        entity.title = ev.summary
                        entity.startDate = ev.startDate
                        entity.endDate = ev.endDate
                        entity.allDay = ev.isAllDay
                        entity.location = ev.location
                        entity.notes = ev.notes
                        entity.lastUpdated = Date()
                        applyICloudProviderFields(to: entity)
                    } else {
                        ...
                        entity.lastUpdated = Date()
                        applyICloudProviderFields(to: entity)
                        if let newID = entity.id { mapBox.set(mapKey, newID.uuidString) }
                    }
                }
                try? context.save()
                let mapSnapshot = mapBox.snapshot
                Task { @MainActor [weak self] in
                    guard let self else { return }
                    var newMap = self.iCloudSyncMap
                    for (k, v) in mapSnapshot { newMap[k] = v }
                    self.iCloudSyncMap = newMap
                    self.notifyCalendarDidChangeMainActor()
                }
                continuation.resume()
            }
    ...
    private func purgeiCloudEventsFromCoreData() {
