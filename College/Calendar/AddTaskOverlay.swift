...
struct AddTaskOverlay: View {
    ...
    let prefillCourseID: NSManagedObjectID?
    ...
    private struct CategoryOption: Identifiable, Hashable {
        ...
        var gradingCategoryID: NSManagedObjectID?
    ...
    @State private var selectedGradingCategoryID: NSManagedObjectID? = nil
    ...
    private struct Snapshot: Equatable {
        ...
        let gradingCategoryID: NSManagedObjectID?
    ...
    private var courses: [CourseEntity] {
        ...
        request.sortDescriptors = [
            NSSortDescriptor(keyPath: \CourseEntity.code, ascending: true),
            NSSortDescriptor(keyPath: \CourseEntity.name, ascending: true)
        ]
    ...
    init(
        ...
        prefillCourseID: NSManagedObjectID? = nil,
        ...
        if let taskToEdit {
            ...
            let storedWeightPercent: String = {
                let w = (taskToEdit.value(forKey: "weightPercent") as? Double) ?? (taskToEdit.value(forKey: "weightPercent") as? NSNumber)?.doubleValue
                if let w, w > 0 {
            ...
            let storedEffort: (hours: String, minutes: String) = {
                let total = (taskToEdit.value(forKey: "estimatedEffortMinutes") as? Int32).map(Int.init)
                    ?? (taskToEdit.value(forKey: "estimatedEffortMinutes") as? Int).map { $0 }
                    ?? (taskToEdit.value(forKey: "estimatedEffortMinutes") as? NSNumber)?.intValue
            ...
            _selectedGradingCategoryID = State(initialValue: taskToEdit.gradingCategory?.objectID)
            ...
            initialSnapshot = Snapshot(
                ...
                gradingCategoryID: taskToEdit.gradingCategory?.objectID,
    ...
    private var categoryOptions: [CategoryOption] {
        ...
        let categories = coreDataManager.gradingCategories(for: course)
        ...
        return categories.map { cat in
            CategoryOption(
                ...
                weightPercent: (cat.value(forKey: "weightPercent") as? Double) ?? (cat.value(forKey: "weightPercent") as? NSNumber)?.doubleValue,
                gradingCategoryID: cat.objectID
            )
        }
    }
    ...
    private func save() {
        ...
        let gradingCategory: CourseGradingCategoryEntity? = {
            guard let id = selectedGradingCategoryID else { return nil }
            return (try? coreDataManager.viewContext.existingObject(with: id)) as? CourseGradingCategoryEntity
        }()
        ...
        let categoryWeight = gradingCategory.flatMap {
            ($0.value(forKey: "weightPercent") as? Double) ?? ($0.value(forKey: "weightPercent") as? NSNumber)?.doubleValue
        }
