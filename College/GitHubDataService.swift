...
class GitHubDataService {
    ...
    func fetchSchoolsList() async throws -> [SchoolManifest] {
        let primaryURL = URL(string: "\(fullBaseURL)/manifests/schools.json")!
        let fallbackURL = URL(string: "\(fallbackBaseURL)/manifests/schools.json")!
    ...
    func downloadSchoolProfile(schoolID: String) async throws -> SchoolProfile {
        let primaryURL = URL(string: "\(fullBaseURL)/profiles/\(schoolID).json")!
        let fallbackURL = URL(string: "\(fallbackBaseURL)/profiles/\(schoolID).json")!
        let (data, httpResponse, usedURL) = try await fetchDataWithFallback(primaryURL: primaryURL, fallbackURL: fallbackURL)
    ...
    func fetchScraperRecipe(format: String) async throws -> ScraperRecipe {
        let primaryURL = URL(string: "\(fullBaseURL)/recipes/\(format)_scraper.json")!
        let fallbackURL = URL(string: "\(fallbackBaseURL)/recipes/\(format)_scraper.json")!
        let (data, httpResponse, usedURL) = try await fetchDataWithFallback(primaryURL: primaryURL, fallbackURL: fallbackURL)
