...
struct DocumentsView: View {
    ...
    @State private var selectedDocumentIDs: Set<NSManagedObjectID> = []
    ...
    private var documentsSummaryLine: String {
        let f = vaultMetrics.totalFiles
        let c = vaultMetrics.totalCategories
        if f == 1, c == 1 {
            return String(localized: "documents.summary_one_file_one_category")
        }
        if f == 1 {
            return String(format: String(localized: "documents.summary_one_file"), c)
        }
        if c == 1 {
            return String(format: String(localized: "documents.summary_one_category"), f)
        }
        return String(format: String(localized: "documents.summary_format"), f, c)
    }

    private var documentsHeroTitle: String {
        "Documents"
    }
    ...
    private var sidebarFolderItems: [VaultSidebarItem] {
        coreDataManager.fetchRootVaultFolders().compactMap { folder in
            guard let id = folder.id else { return nil }
            let name = (folder.fileName ?? "Folder").trimmingCharacters(in: .whitespacesAndNewlines)
            return .folder(id, name.isEmpty ? "Folder" : name)
        }
    }

    private var allFolderEntities: [VaultDocumentEntity] {
        coreDataManager.vaultDocuments
            .filter { $0.isFolder }
            .sorted {
    ...
    private struct VaultTreeItem: Identifiable {
        ...
        let entity: VaultDocumentEntity
        var children: [VaultTreeItem]?   // nil = leaf (file), array = folder

        init(entity: VaultDocumentEntity, children: [VaultTreeItem]? = nil) {
            self.id = entity.objectID
            self.entity = entity
            self.children = children
        }

        var name: String { entity.fileName ?? "Untitled" }
    ...
    private func buildFinderTreeItems(parentID: UUID?) -> [VaultTreeItem] {
        let all = coreDataManager.vaultDocuments
    ...
    private func childFolders(of parentID: UUID?) -> [VaultDocumentEntity] {
    ...
    private var selectedDocument: VaultDocumentEntity? {
        guard let selectedID = selectedDocumentIDs.first else { return nil }
        return coreDataManager.vaultDocuments.first(where: { $0.objectID == selectedID })
    }

    private var selectedDocuments: [VaultDocumentEntity] {
        coreDataManager.vaultDocuments.filter { doc in
            !doc.isFolder && selectedDocumentIDs.contains(doc.objectID)
        }
    }
    ...
    private var visibleVaultDocuments: [VaultDocumentEntity] {
        let query = searchText.trimmingCharacters(in: .whitespacesAndNewlines)
        return coreDataManager.vaultDocuments.filter { doc in
    ...
    var body: some View {
        GeometryReader { proxy in
            ...
            VStack(alignment: .leading, spacing: 16) {
                ...
                filesListSection
                    ...
                    .modifier(DocumentsEntranceModifier(index: 4, isVisible: hasAnimatedIn, reduceMotion: motionReduced))
            }
        ...
        .onReceive(coreDataManager.$vaultDocuments) { _ in
            recomputeVaultMetrics()
            selectedDocumentIDs = selectedDocumentIDs.filter { id in
                visibleVaultDocuments.contains(where: { $0.objectID == id })
            }
        }
        ...
        .confirmationDialog(
            ...
            if let pendingDeleteFolderID,
               let folder = folderEntity(withID: pendingDeleteFolderID) {
                Text("Choose how to delete \(folder.fileName ?? "this folder").")
            } else {
    ...
    private func recomputeVaultMetrics() {
        ...
        let docs = coreDataManager.vaultDocuments.filter { !$0.isFolder }
        snapshot.totalFiles = docs.count
        for doc in docs {
            snapshot.totalBytes += doc.fileSizeBytes
            if let c = doc.category { categories.insert(c) }
    ...
    @MainActor
    private func openVaultDocument(_ doc: VaultDocumentEntity) {
        Task {
            guard let url = await coreDataManager.decryptedTempURLForVaultDocument(doc) else { return }
            coreDataManager.markVaultDocumentOpened(doc)
            NSWorkspace.shared.open(url)
        }
    }

    @MainActor
    private func revealVaultDocumentInFinder(_ doc: VaultDocumentEntity) {
        guard let vaultURL = coreDataManager.urlForVaultDocument(doc) else { return }
        NSWorkspace.shared.selectFile(vaultURL.path, inFileViewerRootedAtPath: "")
    }

    private var availableCourseCodes: [String] {
        let codes = coreDataManager.semesters
    ...
    #if os(macOS)
    @MainActor
    private func quickLookVaultDocument(_ doc: VaultDocumentEntity) {
        Task {
            guard let url = await coreDataManager.decryptedTempURLForVaultDocument(doc) else { return }
    ...
    private var headerSection: some View {
        UnifiedActionHeader(
            title: documentsHeroTitle,
            subtitle: documentsSummaryLine,
            topPadding: 0,
            horizontalPadding: 0,
            bottomPadding: 0,
        ...
        .animation(.easeInOut(duration: 0.2), value: vaultMetrics.totalFiles)
    }
    ...
    private var inspectorSection: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            if let document = selectedDocument {
                Group {
                    inspectorRow(label: "Name", value: document.fileName ?? "Document")
                    inspectorRow(label: "Category", value: (document.category ?? "Other"))
    ...
    private func createFolderFromPrompt() {
        ...
        let created = coreDataManager.createVaultFolder(name: trimmed, parentFolderID: selectedSidebarItem.folderID)
        if let created, let id = created.id {
            selectedSidebarItem = .folder(id, trimmed)
        }
        newFolderName = ""
    }

    private func folderEntity(withID id: UUID) -> VaultDocumentEntity? {
        coreDataManager.vaultDocuments.first(where: { $0.isFolder && $0.id == id })
    }
    ...
    private func renameFolderFromPrompt() {
        ...
        coreDataManager.renameVaultFolder(folder, newName: trimmed)
    ...
    private func deletePendingFolder(includeContents: Bool) {
        ...
        coreDataManager.deleteVaultFolder(folder, includeContents: includeContents)
    ...
    private func dragPayload(for doc: VaultDocumentEntity) -> String {
        let ids: [NSManagedObjectID]
        if selectedDocumentIDs.contains(doc.objectID), !selectedDocumentIDs.isEmpty {
    ...
    private func moveDraggedDocuments(_ droppedPayloads: [String], to folderID: UUID?) {
        ...
        let objectIDs: [NSManagedObjectID] = tokens.compactMap { payload in
                guard let uri = URL(string: payload) else { return nil }
                return coreDataManager.container.persistentStoreCoordinator.managedObjectID(forURIRepresentation: uri)
            }

        guard !objectIDs.isEmpty else { return }

        for objectID in objectIDs {
            guard let doc = try? coreDataManager.viewContext.existingObject(with: objectID) as? VaultDocumentEntity else {
                continue
            }
            guard !doc.isFolder, doc.parentFolderID != folderID else { continue }
            coreDataManager.moveVaultDocument(doc, toFolderID: folderID)
        }
    }

    private func selectAllVisibleDocuments() {
        selectedDocumentIDs = Set(visibleVaultDocuments.map(\.objectID))
    }
    ...
    private func moveSelectedDocuments(to folderID: UUID?) {
        ...
        selectedDocumentIDs = Set(selectedDocuments.map(\.objectID))
    }
    ...
    private func deleteSelectedDocumentsConfirmed() {
        ...
        coreDataManager.bulkDeleteVaultDocuments(docs)
        selectedDocumentIDs = []
    }
    ...
    @ViewBuilder
    private func vaultDocumentContextMenu(for doc: VaultDocumentEntity) -> some View {
        Button("Open") {
        ...
        Menu("Move to Folder") {
            Button("All Files (Root)") {
                coreDataManager.moveVaultDocument(doc, toFolderID: nil)
            }
            ...
            ForEach(moveDestinationFolders, id: \.id) { destination in
                Button(destination.name) {
                    coreDataManager.moveVaultDocument(doc, toFolderID: destination.id)
                }
            ...
            if let linked = doc.courseCodeLinked?.trimmingCharacters(in: .whitespacesAndNewlines),
               !linked.isEmpty {
                Button("Remove Link (\(linked))") {
                    coreDataManager.setVaultDocumentCourseLink(doc, courseCode: nil)
                }
            }
            ForEach(Array(availableCourseCodes.prefix(30)), id: \.self) { code in
                Button(code) {
                    coreDataManager.setVaultDocumentCourseLink(doc, courseCode: code)
                }
            }
        }
        Divider()
    ...
    private var vaultDocumentsGrid: some View {
        ScrollView {
            LazyVGrid(
                ...
                ForEach(visibleVaultDocuments, id: \.objectID) { doc in
                    VaultDocumentGridCell(doc: doc, isSelected: selectedDocumentIDs.contains(doc.objectID))
                        ...
                        .onTapGesture {
                            selectedDocumentIDs = [doc.objectID]
                        }
                        .contextMenu {
    ...
    private var finderStyleTable: some View {
        Table(of: VaultTreeItem.self, selection: $selectedDocumentIDs) {
            TableColumn("Name") { item in
                finderNameCell(item: item)
                    ...
                    .onTapGesture(count: 2) {
                        if item.isFolder {
                            if let id = item.entity.id {
                                let name = (item.entity.fileName ?? "Folder")
                                    .trimmingCharacters(in: .whitespacesAndNewlines)
                                selectedSidebarItem = .folder(id, name.isEmpty ? "Folder" : name)
                            }
                        } else {
        ...
        .contextMenu(forSelectionType: NSManagedObjectID.self) { ids in
            if let id = ids.first,
               let doc = coreDataManager.vaultDocuments.first(where: { $0.objectID == id }) {
                vaultDocumentContextMenu(for: doc)
            }
        }
    }
                    ...
                    if finderRootItems.isEmpty {
                        ...
                        .onKeyPress(.space) {
                            guard let selectedDocumentID = selectedDocumentIDs.first,
                                  let doc = coreDataManager.vaultDocuments.first(where: { $0.objectID == selectedDocumentID }) else {
                                return .ignored
                            }
                            quickLookVaultDocument(doc)
                            return .handled
...
private struct VaultDocumentPresentation {
    let doc: VaultDocumentEntity

    var fileName: String {
        doc.fileName ?? String(localized: "documents.unknown_file")
    }
    ...
    var categoryText: String {
        (doc.category ?? "UNCATEGORIZED").uppercased()
    }
    ...
    var dateString: String {
        guard let addedAt = doc.addedAt else { return String(localized: "documents.unknown_date") }
        let formatter = DateFormatter()
        formatter.dateStyle = .medium
        formatter.timeStyle = .short
        ...
        LaunchPreloadCoordinator.registerFeaturePreload(
            .init(
                ...
                run: { context, onProgress, _ in
                    context.coreDataManager.fetchVaultDocuments()
                    onProgress(1)
                }
            )
        )
