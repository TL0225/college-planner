...
struct DocumentsWidget: View {
    ...
    private static var recentDocsRequest: NSFetchRequest<VaultDocumentEntity> {
        let r = NSFetchRequest<VaultDocumentEntity>(entityName: "VaultDocumentEntity")
        r.sortDescriptors = [
            NSSortDescriptor(keyPath: \VaultDocumentEntity.lastOpenedAt, ascending: false),
            NSSortDescriptor(keyPath: \VaultDocumentEntity.addedAt, ascending: false)
        ]
        r.fetchLimit = 3
        return r
    }

    @FetchRequest(fetchRequest: DocumentsWidget.recentDocsRequest)
    private var recentDocuments: FetchedResults<VaultDocumentEntity>
    ...
    var body: some View {
        OverviewCard {
            ...
            if recentDocuments.isEmpty {
                ...
                VStack(spacing: 8) {
                    ForEach(Array(recentDocuments.prefix(3).enumerated()), id: \.offset) { _, doc in
                        documentRow(doc)
                    }
                }
            }
    ...
    private func documentRow(_ doc: VaultDocumentEntity) -> some View {
        let (iconName, iconColor, iconBg) = documentIconInfo(doc)
        let displayName = doc.customDisplayName ?? doc.fileName ?? "Document"
    ...
    private func documentIconInfo(_ doc: VaultDocumentEntity) -> (String, Color, Color) {
    ...
    private func relativeDocumentDate(_ doc: VaultDocumentEntity) -> String {
