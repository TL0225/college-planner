...
struct DeadlinesWidget: View {
    ...
    private var deadlines: [TaskEntity] { Array(pendingTasks.prefix(4)) }
    ...
    var body: some View {
        if deadlines.isEmpty {
            ...
            OverviewCard {
                ...
                VStack(spacing: 0) {
                    ForEach(Array(deadlines.enumerated()), id: \.offset) { _, task in
                        deadlineRow(task: task)
                        if task != deadlines.last {
    ...
    private func deadlineRow(task: TaskEntity) -> some View {
    ...
    private func taskUrgency(_ task: TaskEntity) -> TaskUrgency {
        guard let due = task.dueDate else {
            return TaskUrgency(dotColor: Color(hex: "9CA3AF"), label: "NO DATE",
                               labelColor: Color(hex: "6B7280"), labelBg: Color(hex: "F3F4F6"))
        }
