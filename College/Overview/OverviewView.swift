        ...
        LaunchPreloadCoordinator.registerFeaturePreload(
            .init(
                ...
                run: { context, onProgress, _ in
                    context.coreDataManager.fetchSemesters()
                    onProgress(0.33)
                    context.coreDataManager.fetchPlans()
                    onProgress(0.66)
                    context.coreDataManager.fetchProfile()
                    onProgress(1)
                }
            )
        )
...
struct OverviewView: View {
    ...
    private enum OverviewDeclaredProgramKind {
        ...
        func creditsProgress(coreData: CoreDataManager, displayName: String) -> CoreDataManager.CreditsProgressSummary {
            switch self {
            case .major:
                return coreData.majorRequirementsCreditsProgress(forMajorDisplay: displayName)
            case .minor:
                return coreData.minorRequirementsCreditsProgress(forMinorDisplay: displayName)
            }
        }
    ...
    private var currentResolvedSemester: SemesterEntity? {
        let activePlanSemesters = coreDataManager.getActivePlan()?.semestersArray ?? []
        let source = activePlanSemesters.isEmpty ? coreDataManager.semesters : activePlanSemesters
        ...
        HStack(spacing: 24) {
            if names.isEmpty {
                ...
                ForEach(0..<(maxC - 1), id: \.self) { slot in
                    let name = names[slot]
                    let progress = kind.creditsProgress(coreData: coreDataManager, displayName: name)
                    degreeProgramRow(
                        isConfigured: true,
    ...
    private func presentAddCourse() {
        ...
        if let objectID = currentSemester?.objectID {
            modalCoordinator.activeModal = .addCatalogCourse(semesterObjectID: objectID)
        } else {
    ...
    private var header: some View {
        ...
        let firstName = coreDataManager.profile?.name?.components(separatedBy: " ").first?.trimmingCharacters(in: .whitespacesAndNewlines)
        return UnifiedActionHeader(
            title: firstName.map { "Welcome back, \($0)." } ?? "Welcome back.",
            subtitle: termText,
    ...
    private func presentCombinedSyllabusQuickLook() async {
        ...
        let currentSemester = currentResolvedSemester
        let activeCourses = (currentSemester?.courses?.allObjects as? [CourseEntity]) ?? []
        let activeCodes = Set(activeCourses.compactMap { $0.code?.trimmingCharacters(in: .whitespacesAndNewlines).uppercased() })

        let syllabusDocs = coreDataManager.vaultDocuments.filter { doc in
            let category = (doc.category ?? "").lowercased()
            if category != VaultDocumentCategory.syllabi.rawValue.lowercased() { return false }
            guard !activeCodes.isEmpty else { return true }
            let linked = (doc.courseCodeLinked ?? "").trimmingCharacters(in: .whitespacesAndNewlines).uppercased()
            return linked.isEmpty || activeCodes.contains(linked)
        }
        ...
        for doc in syllabusDocs {
            guard let url = await coreDataManager.decryptedTempURLForVaultDocument(doc) else { continue }
        ...
        let previewURL = FileManager.default.temporaryDirectory.appendingPathComponent("CombinedSyllabus-\(UUID().uuidString).pdf")
        if merged.write(to: previewURL) {
            #if os(macOS)
            OverviewQuickLookPresenter.shared.present(url: previewURL)
            #endif
        } else {
            appNotifications.post(kind: .error, title: "Quick Look Failed", message: "Unable to generate combined syllabus PDF.")
        }
    }
}
...
private struct NextUpPill: View {
    ...
    @FetchRequest(
        ...
        predicate: NSPredicate(format: "startDate >= %@", Date() as CVarArg)
    ) private var upcomingEvents: FetchedResults<CalendarEventEntity>
    @State private var phase: CGFloat = 0
            ...
            HStack(spacing: 16) {
                HStack(spacing: 12) {
                    ...
                    VStack(alignment: .leading, spacing: 2) {
                        ...
                        Text("\(nextEvent.title ?? "Event") (\(Self.nextUpFormatter.string(from: nextEvent.startDate ?? Date())))")
                            .font(.system(size: 14, weight: .bold))
                            .foregroundStyle(.primary)
                    }
                }
...
private struct AcademicStandingCard: View {
    ...
    private var programRows: [ProgramStandingRow.RowData] {
        let majorColors: [Color] = [Color.accentColor, .orange, .green]
        let minorColors: [Color] = [.purple, .pink, .teal]
        var rows: [ProgramStandingRow.RowData] = []

        for (idx, majorName) in coreDataManager.resolvedMajorNames().enumerated() {
            let reqs = coreDataManager.getDegreeRequirementsForMajorDisplay(majorName)
            let gpaSummary = coreDataManager.majorGPASummary(requirements: reqs)
            let creditProgress = coreDataManager.majorRequirementsCreditsProgress(forMajorDisplay: majorName)
        ...
        for (idx, minorName) in coreDataManager.resolvedMinorNames().enumerated() {
            let gpaSummary = coreDataManager.minorGPASummary(minorName: minorName)
            let creditProgress = coreDataManager.minorRequirementsCreditsProgress(forMinorDisplay: minorName)
            rows.append(ProgramStandingRow.RowData(
                id: "minor-\(idx)",
                name: minorName,
                shortLabel: "MINOR",
    ...
    private var gpaMetrics: GPAMetrics {
        let plannerGPA = academicMetricsStore.snapshot?.cumulativeGPA
        let storedPrimaryGPA = coreDataManager.profile?.gpa ?? 0
        ...
        let storedSchoolGPA = coreDataManager.profile?.schoolGpa ?? 0
                ...
                VStack(alignment: .leading, spacing: 22) {
                    ...
                    HStack(alignment: .firstTextBaseline, spacing: 6) {
                        AnimatedMetricValueText(
                            value: gpaMetrics.primaryValue,
                            fallback: gpaMetrics.primaryDisplay,
                        ...
                        VStack(alignment: .trailing, spacing: 2) {
                            Text("CUMULATIVE GPA")
                                ...
                                .foregroundStyle(.secondary)
                            if let n = academicMetricsStore.snapshot?.gpaCoursesCounted, n > 0 {
                    ...
                    // Per-program GPA + credit progress breakdown
                    if !programRows.isEmpty {
                        VStack(spacing: 10) {
                            ForEach(programRows) { row in
                    ...
                    let achievements = (coreDataManager.profile?.achievements?.allObjects as? [AchievementEntity]) ?? []
...
private struct ProgramStandingRow: View {
    struct RowData: Identifiable {
        ...
        let gpa: Double?
        let creditProgress: CoreDataManager.CreditsProgressSummary
        let color: Color
        let isMinor: Bool
    }
...
private struct UnifiedTimelineCard: View {
    ...
    @FetchRequest private var todayEvents: FetchedResults<CalendarEventEntity>
    ...
    private var activeTimelineRowID: String? {
        todayEvents.first(where: { ($0.endDate ?? $0.startDate ?? Date()) >= currentTime })
            .map(timelineStableID(for:))
    }

    private func timelineStableID(for event: CalendarEventEntity) -> String {
        if let id = event.id {
    ...
    init() {
        ...
        _todayEvents = FetchRequest(
            entity: CalendarEventEntity.entity(),
            sortDescriptors: [NSSortDescriptor(keyPath: \CalendarEventEntity.startDate, ascending: true)],
            predicate: NSPredicate(format: "startDate >= %@ AND startDate < %@", startOfDay as NSDate, endOfDay as NSDate)
        )
    }
                    ...
                    ScrollViewReader { proxy in
                        ScrollView(.vertical, showsIndicators: false) {
                            LazyVStack(spacing: 8) { // Reduced spacing between items to connect the line better
                                let firstActiveID = todayEvents.first(where: { ($0.endDate ?? $0.startDate ?? Date()) >= currentTime })
                                    .map(timelineStableID(for:))
                                ForEach(todayEvents, id: \.objectID) { event in
    ...
    @ViewBuilder
    private func timelineEventRow(event: CalendarEventEntity, isFirstActive: Bool) -> some View {
...
private struct ActiveCoursesCard: View {
    ...
    @State private var cachedVisibleCourses: [CourseEntity] = []
    ...
    private func requirementText(for course: CourseEntity) -> String {
    ...
    private func applyCourseFilter(courses: [CourseEntity], query: String) -> [CourseEntity] {
    ...
    private func scheduleVisibleCourseRefresh(courses: [CourseEntity], animated: Bool) {
    ...
    var body: some View {
        let activePlanSemesters = coreDataManager.getActivePlan()?.semestersArray ?? []
        let sourceSemesters = activePlanSemesters.isEmpty ? coreDataManager.semesters : activePlanSemesters
