...
struct SyllabusReviewView: View {
    ...
    let overrideEntity: CourseOverrideEntity?
    let plannedCourse: CourseEntity?
    let onClose: () -> Void
    ...
    var body: some View {
        ...
        .onChange(of: sortAscending) { _, _ in
            // Re-sort without re-fetching CoreData.
    ...
    private func persistGradingIfPossible() {
        ...
        let upserted = coreDataManager.upsertGradingCategories(for: plannedCourse, items: vm.syllabusData.grading, source: "syllabus_ai")
    ...
    private func persistInstructorIfPossible() {
        ...
        let didWrite = coreDataManager.upsertCourseInstructorContact(
    ...
    private func refreshConflicts() {
        ...
        let existing = coreDataManager.fetchCalendarEvents(semester: semester, start: minStart, end: maxEnd)
        ...
        for draft in vm.draftEvents {
            ...
            let overlaps = existing.compactMap { e -> ConflictEvent? in
                guard let s = e.startDate, let end = e.endDate else { return nil }
                // Skip if the existing event is all-day — no time conflict possible.
                guard !e.allDay else { return nil }
                guard s < draft.endDate, end > draft.startDate else { return nil }
    ...
    /// Debounced wrapper — avoids a CoreData fetch + O(n²) scan on every single checkbox toggle.
    ...
    private func addSelectedEvents() {
        ...
        var created: [CalendarEventEntity] = []
        for ev in selected {
            ...
            let alreadyExists = coreDataManager.viewContext
                .registeredObjects
                .compactMap { $0 as? CalendarEventEntity }
                .contains(where: { $0.title == expectedTitle && $0.startDate == expectedStart })
                || ({
                    let req = CalendarEventEntity.fetchRequest() as NSFetchRequest<CalendarEventEntity>
                    req.predicate = NSPredicate(
                        format: "title == %@ AND startDate == %@",
                        expectedTitle, expectedStart as NSDate
                    )
                    req.fetchLimit = 1
                    return (try? coreDataManager.viewContext.fetch(req))?.isEmpty == false
                }())
            ...
            let e = coreDataManager.addCalendarEvent(
        ...
        for e in created {
            calendarManager.exportEventToAppleCalendar(e, calendarName: courseCode)
        }
