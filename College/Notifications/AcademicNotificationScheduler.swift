...
final class AcademicNotificationScheduler: @unchecked Sendable {
    ...
    func reschedule(using coreDataManager: CoreDataManager) {
        let gpa = coreDataManager.profile?.gpa ?? 0
        let semesters = coreDataManager.semesters.map {
            SemesterSnapshot(
                name: $0.name,
                year: Int($0.year),
                season: $0.season,
        ...
        UNUserNotificationCenter.current().requestAuthorization(options: [.alert, .sound]) { granted, _ in
            ...
            // Remove old academic notifications before rescheduling
