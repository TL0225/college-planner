...
class ModernCampusEngine {
	...
	private static func classifySidebarForCoursesWithLLM(navoids: [String: String], logger: DebugLogger) async -> String? {
		// Ensure Gemma 4 is installed, downloading if not yet present.
...
"""
	...
	///
	/// This helper returns the **latest** catalogs only: newest academic year in the title when
	/// present, otherwise the gateway’s **publication order** (UB lists newest first) and `catoid`
	/// as a tie-breaker. One pick per known keyword, with no prior-year or “all catalogs this year” expansion.
	...
	private static func discoverBuffaloActiveCatalogs(baseURL: String) async throws -> [ModernCampusCatalogDescriptor] {
		...
		/// `listIndex` preserves `catalog_list.php` DOM order; UB lists the newest catalogs first.
		var candidates: [(listIndex: Int, catoid: String, text: String)] = []
		candidates.reserveCapacity(anchors.count)

		for (listIndex, a) in anchors.enumerated() {
			...
			candidates.append((listIndex: listIndex, catoid: catoid, text: text))
		}

		func catoidNumeric(_ raw: String) -> Int {
			Int(raw.trimmingCharacters(in: .whitespacesAndNewlines)) ?? 0
		}

		/// Prefer newer academic year, then earlier list position (newest-published on UB’s gateway), then higher catoid.
		func prefersMoreRecentCatalog(
			_ a: (listIndex: Int, catoid: String, text: String),
			yearA: (start: Int, end: Int)?,
			over b: (listIndex: Int, catoid: String, text: String),
			yearB: (start: Int, end: Int)?
		) -> Bool {
			switch (yearA, yearB) {
			case let (ya?, yb?):
				if ya.end != yb.end { return ya.end > yb.end }
				if a.listIndex != b.listIndex { return a.listIndex < b.listIndex }
				return catoidNumeric(a.catoid) > catoidNumeric(b.catoid)
			case (_?, nil):
				return true
			case (nil, _?):
				return false
		...
		let yearfulCandidates = candidates.compactMap { cand -> (cand: (listIndex: Int, catoid: String, text: String), year: (start: Int, end: Int))? in
			guard let y = parseAcademicYear(cand.text) else { return nil }
			return (cand: cand, year: y)
		}
		let current: (start: Int, end: Int)? = {
			guard let first = yearfulCandidates.first else { return nil }
			let best = yearfulCandidates.dropFirst().reduce(first) { acc, next in
				prefersMoreRecentCatalog(next.cand, yearA: next.year, over: acc.cand, yearB: acc.year) ? next : acc
			}
			return best.year
		}()
		...
		for (keyword, defaultTitle) in requiredKeywords {
			...
			let chosen: (listIndex: Int, catoid: String, text: String) = {
				let scored = matches.map { cand in
					(cand: cand, year: parseAcademicYear(cand.text))
				}
				let first = scored[0]
				let best = scored.dropFirst().reduce(first) { acc, next in
					prefersMoreRecentCatalog(next.cand, yearA: next.year, over: acc.cand, yearB: acc.year) ? next : acc
				}
				return best.cand
			}()
		...
		// Drop any keyword pick that clearly belongs to an older academic year (titles without a
		// parseable range are kept so we do not accidentally empty the list).
		if let current {
			selected.removeAll { desc in
				guard let y = parseAcademicYear(desc.title) else { return false }
	...
	nonisolated private static func parseProgramHTML(_ html: String, baseURL: String) -> [ScrapedProgram] {
		do {
			let doc = try SwiftSoup.parse(html)
			let content = try doc.select("#acalog-content").first() ?? doc
			let anchors = try content.select("a[href*=preview_program.php],a[href*=preview_program_nopop.php]")
			...
			for a in anchors.array() {
				...
				programs.append(ScrapedProgram(name: name, type: type, url: absoluteURL))
			}
			return programs
		} catch {
			return []
