...
actor AIAssistantService {
    ...
    private static let preferredAssistantSpecs: [ModelSpec] = [.gemma4]
    ...
    func planResponse(
        ...
        if let reason = await preflightFailureReason() {
            ...
            return PlanningOutcome(
                action: nil,
                fallbackReply: fallbackReply(for: role, message: trimmed, contextSummary: contextSummary, reason: reason, policyContext: policyContext),
                failureReason: reason,
                diagnostics: "preflight"
            )
        }
        ...
        do {
            ...
            var action = parseModelActionIncremental(from: raw, allowedToolNames: allowedPlanningToolNames)
                ?? parseModelAction(from: raw, allowedToolNames: allowedPlanningToolNames)
            if action != nil {
            ...
            if action == nil, AssistantJSONRobustnessSettings.isRepairGenerationEnabled {
                ...
                if let repairRaw = try? await runModel(prompt: repairPrompt, modelPath: modelPath, maxTokens: repairMax),
                   let repaired = parseModelAction(from: repairRaw, allowedToolNames: allowedPlanningToolNames) {
                    action = repaired
    ...
    func preflightFailureReason() async -> FailureReason? {
        ...
        let modelInstalled = await ModelManager.shared.isModelInstalled(.gemma4)

        if !localEnabled, modelInstalled {
            UserDefaults.standard.set(true, forKey: Self.localLLMEnabledKey)
            localEnabled = true
            log("AIAssistantService detected installed Gemma; auto-enabled local LLM runtime")
        }
...
"""
    ...
    private func fallbackActionLine(_ reason: FailureReason) -> String {
        switch reason {
        case .disabled:
            return "Enable Gemma 4 in Settings > AI & Storage, then retry."
        case .modelNotInstalled:
            return "Install Gemma 4 in Settings > AI & Storage, then retry."
        case .modelEnsureFailed:
            return "Retry once; if this repeats, reinstall Gemma 4 from Settings > AI & Storage."
        case .generationFailed:
        ...
        case .unknown:
            return "Retry once; if this repeats, verify Gemma 4 status in Settings > AI & Storage."
        }
    }
...
"""
    ...
    private func runModel(
        ...
        if let onChunk {
            ...
            return try await LocalLLMRunner.shared.generateJSONWithOptionalImagesStreaming(
                prompt: prompt,
                imageURLs: imageURLs,
                modelPath: modelPath,
                maxTokens: maxTokens,
        ...
        return try await LocalLLMRunner.shared.generateJSONWithOptionalImages(
            prompt: prompt,
            imageURLs: imageURLs,
            modelPath: modelPath,
            maxTokens: maxTokens
    ...
    private func ensurePreferredAssistantModelInstalled() async throws -> URL {
        ...
        throw lastError ?? ModelManagerError.downloadFailed("Gemma 4")
    }
}