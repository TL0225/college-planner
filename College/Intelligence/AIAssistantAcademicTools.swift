...
struct GetStudentProfileTool: AIAssistantTool {
    let descriptor = AssistantToolDescriptor(
        ...
        sourceLabel: "ProfileEntity + CoreDataManager"
    )
    ...
    func execute(
        ...
        guard let profile = context.coreDataManager.profile else {
        ...
        let payload = StudentProfilePayload(
            ...
            gpa: profile.gpa,
            creditsEarned: Int(profile.creditsEarned),
            creditsRequired: profile.creditsRequired > 0 ? Int(profile.creditsRequired) : 120,
...
struct GetProgramProgressTool: AIAssistantTool {
    let descriptor = AssistantToolDescriptor(
        ...
        sourceLabel: "AssistantPlannerSnapshot + CoreDataManager progress helpers"
    )
...
struct GetSAPStatusTool: AIAssistantTool {
    let descriptor = AssistantToolDescriptor(
        ...
        sourceLabel: "CoreDataManager.sapStats"
    )
    ...
    func execute(
        ...
        let sap = context.coreDataManager.sapStats()
        let threshold = 0.67
        let status: String
        if sap.attempted == 0 {
            status = "no_history"
...
struct GetFullTimeStatusTool: AIAssistantTool {
    ...
    func execute(
        ...
        let decoded = try? AssistantJSONValue.decodeObject(Arguments.self, from: arguments)
        let plan = try activePlanOrThrow(context.coreDataManager)
        ...
        let jurisdiction = context.coreDataManager.activeSchoolPolicyMetadata().map { AssistantFinancialAidPolicy.resolveJurisdiction(metadata: $0) }
            ?? AssistantFinancialAidPolicy.resolveJurisdiction(activeUniversityName: context.coreDataManager.getActiveUniversityName())
        let evidence = AssistantPolicyEvidenceStore.evidence(
            for: [.schoolFinancialAid, .enrollmentIntensity, .stateAid],
            jurisdiction: jurisdiction
        )
        let storedThreshold = activePolicies(from: context.coreDataManager)?.minCreditsForFullTime
...
struct SearchCatalogCoursesTool: AIAssistantTool {
    ...
    let descriptor = AssistantToolDescriptor(
        ...
        sourceLabel: "CoreDataManager.searchCatalogCourses"
    )
    ...
    func execute(
        ...
        let results = context.coreDataManager.searchCatalogCourses(query: query, limit: limit)
...
struct CheckPrerequisitesTool: AIAssistantTool {
    ...
    let descriptor = AssistantToolDescriptor(
        ...
        sourceLabel: "CoreDataManager.checkPrerequisites"
    )
    ...
    func execute(
        ...
        let plan = try activePlanOrThrow(context.coreDataManager)
        let catalogMatches = context.coreDataManager.searchCatalogCourses(query: requestedCode, limit: 6)
        ...
        let result = context.coreDataManager.checkPrerequisites(for: course, plan: plan)
        let payload = PrerequisiteCheckPayload(
            courseCode: course.courseCode ?? requestedCode,
            met: result.met,
...
struct GetDegreeAuditTool: AIAssistantTool {
    ...
    func execute(
        ...
        let plan = try activePlanOrThrow(context.coreDataManager)
        guard let audit = context.coreDataManager.getGraduationStatus(for: plan) else {
...
struct ExplainRequirementsTool: AIAssistantTool {
    let descriptor = AssistantToolDescriptor(
        ...
        sourceLabel: "CoreDataManager.degreeAudit + AssistantPlannerSnapshot"
    )
...
struct AssessRegistrationReadinessTool: AIAssistantTool {
    ...
    func execute(
        ...
        let audit = context.coreDataManager.getGraduationStatus(for: plan)
        var blockers: [String] = []
        var warnings: [String] = []
        ...
        if let next = nextSemester(after: context.currentDate, in: plan) ?? semesters.last {
            ...
            if credits < 12 {
                warnings.append("\(next.name ?? "Next semester") has \(credits) planned credits, which may be low for full-time pacing or aid.")
            } else if credits > 18 {
                warnings.append("\(next.name ?? "Next semester") has \(credits) planned credits, which may be an overload.")
            }
        } else {
