...
private struct AssistantMessage: Identifiable, Hashable {
    ...
    /// Serialized local image URLs passed to the model for this turn.
    var visionImageURLStrings: [String] = []
    ...
    init(
        ...
        self.visionImageURLStrings = visionImageURLStrings
...
struct AIAssistantView: View {
    ...
    private let preferredAssistantSpecs: [ModelSpec] = [.gemma4]
    ...
    private var assistantInstallRequiredState: some View {
        VStack(spacing: 18) {
            if isCheckingAssistantModel {
                ...
                Text("The assistant chat is disabled until the local Gemma model is installed.")
                ...
                Button {
                    ...
                    Label(isInstallingAssistantModel ? "Installing…" : "Install Gemma 4", systemImage: "arrow.down.circle")
                ...
                .help("Install Gemma 4 local AI model")
            }
        }
    ...
    private func header(shellTopInset: CGFloat) -> some View {
        let topSpacing = shellTopInset + 6
        return UnifiedActionHeader(
            title: "AI Assistant",
            subtitle: "Planner-integrated copilot",
            titleFont: DesignSystem.Fonts.main(size: 20, weight: .bold),
            subtitleFont: DesignSystem.Fonts.main(size: 11, weight: .medium),
            titleColor: DesignSystem.Colors.textMain,
            subtitleColor: DesignSystem.Colors.textLight,
            topPadding: 0,
            horizontalPadding: 0,
            bottomPadding: 0,
    ...
    private func composer(shellBottomInset: CGFloat) -> some View {
        ...
        return VStack(alignment: .leading, spacing: 8) {
            ...
            HStack(alignment: .bottom, spacing: 10) {
                ...
                #else
                TextField("Ask about planning, deadlines, degree progress, or actions...", text: $draftText, axis: .vertical)
                    ...
                    .onTapGesture {
                        setAssistantBreadcrumb("composer.tap")
                        logAssistant("Assistant composer tapped")
                    }
                    .onSubmit {
                        sendMessage()
                    }
    ...
    private func ingestResult(from userMessage: AssistantMessage) -> AssistantAttachmentIngestor.Result {
        let context = userMessage.attachmentContextBlock ?? ""
        let urls = userMessage.visionImageURLStrings.compactMap { URL(string: $0) }
        return AssistantAttachmentIngestor.Result(
            contextBlock: context,
            visionImageURLs: urls,
    ...
    private func sendMessage() {
        ...
        viewModel.startGeneration {
            ...
            if let userIndex = messages.firstIndex(where: { $0.id == userMessageID }) {
                ...
                messages[userIndex].visionImageURLStrings = ingest.visionImageURLs.map(\.absoluteString)
            }
    ...
    private func setMessageFeedback(messageID: UUID, feedback: AssistantReplyFeedback?) {
        ...
        let university = coreDataManager.getActiveUniversityName()?.trimmingCharacters(in: .whitespacesAndNewlines)
    ...
    private func exportTranscript() {
        let payload = messages.map { message in
            PersistedAssistantMessage(
                ...
                visionImageURLStrings: message.visionImageURLStrings.isEmpty ? nil : message.visionImageURLStrings,
    ...
    private func commitUserMessageEdit(messageID: UUID) {
        ...
        messages[idx] = AssistantMessage(
            ...
            visionImageURLStrings: original.visionImageURLStrings,
    ...
    private func confirmPendingAction(_ action: AssistantPendingAction) {
        ...
        case .editTask:
            guard let uri = action.targetObjectURI,
                  let objectID = resolveObjectID(from: uri),
            ...
            let categoryWeightPercent = (task.value(forKey: "categoryWeightPercent") as? NSNumber)?.doubleValue
            let weightPercent = (task.value(forKey: "weightPercent") as? NSNumber)?.doubleValue
            let estimatedEffortMinutes = (task.value(forKey: "estimatedEffortMinutes") as? NSNumber)?.int32Value
    ...
    private func resolveObjectID(from uri: URL) -> NSManagedObjectID? {
        guard let coordinator = coreDataManager.viewContext.persistentStoreCoordinator else { return nil }
    ...
    private func parsePendingAction(from prompt: String) -> AssistantPendingAction? {
        ...
        if normalized.contains("edit task") || normalized.contains("update task") {
            ...
            let matched = tasks.first { ($0.title ?? "").localizedCaseInsensitiveContains(requested) }
            ...
            return AssistantPendingAction(
                ...
                previousEndDate: nil,
                previousAllDay: nil,
                targetObjectURI: matched?.objectID.uriRepresentation()
            )
        }
        ...
        if normalized.contains("edit event") || normalized.contains("update event") {
            ...
            let matched = events.first { ($0.title ?? "").localizedCaseInsensitiveContains(requested) }
            let existingTitle = matched?.title ?? requested
            let newTitle = parseUpdatedTitle(from: prompt, fallbackTitle: existingTitle)
        ...
        Task {
            do {
                ...
                await MainActor.run {
                    ...
                    assistantInstallDetail = "Installed Gemma 4"
                    hasInstalledAssistantModel = true
                    scheduleInitialMessageLoad()
                }
            } catch {
    ...
    private func generateReply(
        ...
        let snapshot = makePlannerSnapshot()
        ...
        let attachmentImageURLs = ingest.visionImageURLs
        let routerSeesAttachments = hadAttachments || !attachmentImageURLs.isEmpty || !attachmentBlock.isEmpty
    ...
    private func generateToolAwareReply(
        ...
        // This guarantees web lookup still works even when Gemma planning/generation fails.
        if let webQuery = explicitWebSearchQuery(from: prompt) {
            let universityName = coreDataManager.getActiveUniversityName()
...
"""
    ...
    private func makeAssistantPolicyContext(for prompt: String) -> AssistantPolicyContext {
        AssistantPolicyContext.from(
            metadata: coreDataManager.activeSchoolPolicyMetadata(),
            activeUniversityName: coreDataManager.getActiveUniversityName(),
            message: prompt
        )
    }
    ...
    private func makePlannerSnapshot() -> AssistantPlannerSnapshot {
        let eventItems = events
            ...
            .compactMap { event -> AssistantPlannerSnapshot.EventItem? in
            guard let startDate = event.startDate else { return nil }
            return AssistantPlannerSnapshot.EventItem(
        ...
        let majorPrograms = majors.map { majorName in
            let progress = coreDataManager.majorRequirementsCreditsProgress(forMajorDisplay: majorName)
            let buckets = coreDataManager.majorRequirementCreditBuckets(forMajorDisplay: majorName)
            var requirements = coreDataManager.getDegreeRequirementsForMajorDisplay(majorName)
            if requirements.isEmpty {
                let degreeType = (coreDataManager.profile?.degreeType ?? "").trimmingCharacters(in: .whitespacesAndNewlines)
                let profileMajor = (coreDataManager.profile?.major ?? "").trimmingCharacters(in: .whitespacesAndNewlines)
                let resolvedURL: String?
                if majorName == profileMajor {
                    resolvedURL = coreDataManager.resolveSelectedMajorProgramURL()
                } else {
                    resolvedURL = coreDataManager.resolveNonMinorMajorProgramURL(display: majorName)
                }
                if let resolvedURL {
                    requirements = coreDataManager.getDegreeRequirements(programURL: resolvedURL, degreeType: degreeType)
                }
            }
        ...
        let minorPrograms = minors.map { minorName in
            let progress = coreDataManager.minorRequirementsCreditsProgress(forMinorDisplay: minorName)
            let buckets = coreDataManager.minorRequirementCreditBuckets(forMinorDisplay: minorName)
            let requirements: [DegreeRequirementEntity] = {
                guard let programURL = coreDataManager.resolveProgramProgramURL(programDisplay: minorName, isMinor: true) else {
                    return []
                }
                return coreDataManager.getDegreeRequirements(programURL: programURL, degreeType: "Minor")
            }()
            return AssistantPlannerSnapshot.ProgramItem(
                name: minorName,
                kind: .minor,
        ...
        for req in requirements {
            ...
            if selectCount > 0 {
                ...
                if let json = req.selectFromDetailedJSON,
                   let detailed = coreDataManager.decodeDetailedCourseList(json),
                   !detailed.isEmpty {
                    choices = detailed
                    ...
                    let rawCodes = coreDataManager.decodeJSONCourseList(req.selectFromJSON)
                    if !rawCodes.isEmpty {
    ...
    private func makeAssistantContextSummary(from snapshot: AssistantPlannerSnapshot, userPrompt: String) async -> String {
        ...
        let profile = coreDataManager.profile
        let gpa = profile?.gpa ?? 0
        let creditsEarned = Int(profile?.creditsEarned ?? 0)
        let creditsRequired = max(Int(profile?.creditsRequired ?? 120), 0)
        let graduationTarget = profile?.expectedGraduation?.trimmingCharacters(in: .whitespacesAndNewlines)
        let sap = coreDataManager.sapStats()
        let storedFullTimeThreshold = coreDataManager.activeSchoolPolicies()?.minCreditsForFullTime
        let formatter = DateFormatter()
        formatter.dateFormat = "EEE h:mm a"
        ...
        let activeUniversityName = coreDataManager.getActiveUniversityName()
        let schoolMetadata = coreDataManager.activeSchoolPolicyMetadata()
    ...
    private func loadPersistedMessages() -> [AssistantMessage] {
        guard let raw = UserDefaults.standard.string(forKey: messageStoreKey),
        ...
        return persisted.compactMap { item in
            ...
            return AssistantMessage(
                ...
                visionImageURLStrings: item.visionImageURLStrings ?? [],
    ...
    private func persistMessages(_ items: [AssistantMessage]) {
        ...
        let payload = capped.map {
            PersistedAssistantMessage(
                ...
                visionImageURLStrings: $0.visionImageURLStrings.isEmpty ? nil : $0.visionImageURLStrings,
        ...
        LaunchPreloadCoordinator.registerFeaturePreload(
            .init(
                ...
                run: { _, onProgress, onDetail in
                    onDetail("Checking Gemma model")
                    if await ModelManager.shared.isModelInstalled(.gemma4) {
                        onProgress(1)
                        return
                    }
                    ...
                    onDetail("Gemma not installed yet; download continues after launch")
                    onProgress(1)
                }
            )
        )
