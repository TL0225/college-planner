...
@MainActor
func activePlanOrThrow(_ coreDataManager: CoreDataManager) throws -> PlanEntity {
    guard let plan = coreDataManager.getActivePlan() else {
...
@MainActor
func activePolicies(from coreDataManager: CoreDataManager) -> SchoolPolicies? {
    coreDataManager.activeSchoolPolicies()
}

@MainActor
func semesterSummaries(from plan: PlanEntity) -> [SemesterSummaryPayload] {
    plan.semestersArray
        .sorted {
            ...
            return SemesterSummaryPayload(
                ...
                courses: courses.map {
                    CoursePlanPayload(
                        ...
                        status: $0.status ?? ($0.isCompleted ? "Completed" : "Planned"),
                        grade: $0.grade
                    )
                }
            )
...
func sortedPlanSemesters(_ plan: PlanEntity) -> [SemesterEntity] {
    plan.semestersArray
}

func nextSemester(after date: Date, in plan: PlanEntity) -> SemesterEntity? {
    let list = sortedPlanSemesters(plan)
    return list.first { semesterApproxStartDate(year: Int($0.year), season: $0.season) > date }
}

func indexOfSemester(_ semester: SemesterEntity, in list: [SemesterEntity]) -> Int? {
    list.firstIndex { $0.objectID == semester.objectID }
}

func semesterAfter(_ semester: SemesterEntity, in list: [SemesterEntity]) -> SemesterEntity? {
