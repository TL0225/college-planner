...
// Feature: Core
// Purpose: Core module — SecurityManager.
...
/// - Encrypts/decrypts sensitive blobs stored in local store/UserDefaults/files
...
final class SecurityManager: ObservableObject {
    ...
    /// Best-effort migration can be expensive (local store fetch + file IO). Only run once.
    ...
    private func performPostAuthenticationUnlock(unlockStart: Date, source: String) async throws {
        ...
        if !self.didMigrateSensitiveBlobsV1 {
            ...
            let store = CollegePersistence.shared.appDataStore
            Task { @MainActor in
                try? await Task.sleep(nanoseconds: 500_000_000)
                SensitiveBlobMigration.migrateLocalStore(store: store, key: key)
            }
        }
    }
    ...
    /// Installs an ephemeral master key so backup integration tests can run without Keychain unlock.
    func installMasterKeyForUnitTestingIfNeeded() {
...
private enum SensitiveBlobMigration {
    @MainActor
    static func migrateLocalStore(store: AppDataStore, key: SymmetricKey) {
        let ctx = ModelContext(store.profileContainer)
        ctx.autosaveEnabled = false
        var changed = false

        if let profile = try? store.profileRepository.fetchPrimaryProfile() {
            if let d = profile.profilePhotoData, !BlobCrypto.isEncryptedBlob(d),
               let enc = try? BlobCrypto.encryptIfNeeded(d, using: key) {
                profile.profilePhotoData = enc
                changed = true
            }
        }

        if let docs = try? store.vaultRepository.fetchDocuments(limit: 5000) {
            ...
            for doc in docs where !doc.isFolder {
                ...
                guard let bytes = try? Data(contentsOf: storedURL),
                      !BlobCrypto.isEncryptedBlob(bytes),
                      let enc = try? BlobCrypto.encryptIfNeeded(bytes, using: key) else { continue }
