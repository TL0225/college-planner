...
// Feature: Core
// Purpose: Core module — AuthorizedRoot.
...
final class CloudIntegrationService: ObservableObject {
    ...
    private init(collegePersistence: CollegePersistence = .shared) {
    ...
    func refreshDetectedProviders() {
        ...
        let watched = CollegePersistence.shared.fetchWatchedFolders().filter { $0.isEnabled }

        var bookmarkValidityByPath: [String: Bool] = [:]
        for entry in watched {
            let canonical = (entry.path as NSString).standardizingPath
        ...
        for entry in watched {
            let watchedPath = (entry.path as NSString).standardizingPath
    ...
    func revokeAuthorization(for provider: Provider) {
        let watched = CollegePersistence.shared.fetchWatchedFolders()
        for entry in watched {
            let canonical = (entry.path as NSString).standardizingPath
            if provider.authorizedPaths.contains(canonical) {
                CollegePersistence.shared.removeWatchedFolder(id: entry.id)
        ...
        for entry in watched {
            ...
            if isStale,
               let refreshed = try? url.bookmarkData(
                ...
                CollegePersistence.shared.updateWatchedFolderBookmark(id: entry.id, bookmarkData: refreshed)
            }
        }
    }

    private func isBookmarkValid(for entry: WatchedFolder) -> Bool {
