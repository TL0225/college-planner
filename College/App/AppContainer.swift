...
extension View {
    /// Primary ADR 005 injection path plus child keys for views not yet migrated off direct `@Environment` lookups.
    @MainActor
    func appContainerEnvironment(_ container: AppContainer) -> some View {
        environment(container)
            .environment(container.modalCoordinator)
            .environment(container.academicMetricsStore)
            .environment(container.auditSnapshotStore)
            .environment(container.launchPreloadCoordinator)
            .environment(container.appActivity)
    }
}
